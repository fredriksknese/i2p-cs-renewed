...
| 3-14 | `p3/next-hop-filter` | **Diagnosed in session 9, from the 3-13 CI logs; do this next.** With the build reply finally readable, i2pd built 2 tunnels through us out of 529 attempts, and our own log says why: **528 of 531 build decisions are `AcceptingTunnels: Reject due same next destination`** (`TransitTunnelProvider.cs:398`, `NextHopFilter.Update`). In a fixture with two routers there is only one next hop we can ever be asked for, so the anti-enumeration filter refuses nearly every tunnel we are asked to carry — i2pd reads that as `ret code=30` and logs `Outbound tunnel … has been declined` (392 outbound, 71 inbound). The filter is a real defence; do not simply delete it. Settle first: what i2pd throttles instead (per-peer request rate, not next-hop repetition); that the rejection is `LogDebug`, so a Release router declines everything and says nothing — the 0-1 complaint in a third place; and where the other half of the requests go, since 1086 were decrypted against 531 decisions. | A router with one usable next hop still carries tunnels; the accept/reject ratio and its reason are visible at the default log level |
