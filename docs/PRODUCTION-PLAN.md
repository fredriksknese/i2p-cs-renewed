...
| 4-0d-fix | `p4/ssu2-netid-constant` | **4-0d was incomplete — there is a sixth site and its own guard cannot see it.** `SSU2Constants.NETWORK_ID = 2` (`SSU2Constants.cs:29`) is a live-network literal, written raw into the HolePunch header at `SSU2RelayHandler.cs:1163` as `header[14] = SSU2Constants.NETWORK_ID`. `Ssu2NetIdTest`'s regex matches `NetId = 2` and so sees neither the constant nor the raw byte write. `NTCP2Constants.cs:27` declares the same literal — check whether NTCP2 has the same defect. Delete the constants or define them from `I2PConstants.I2PNetworkId`, and widen the guard to catch a raw index write. | Guard catches the constant and the `header[14]` form; both confirmed red first |
...
| 4-0f | `p4/integration-host-crash` | **Found by 4-0e. Two distinct causes, both open, and the integration job is red on `github-master` until they are fixed.** (1) `[CancelAfter(n)]` cancels a `CancellationToken` NUnit passes **as a test-method parameter**, and none of the 23 tests carrying the attribute declares one — so a timeout marks the test failed while its thread runs on, and the orphans take the host down. Two CI runs died this way, on a different hanging test each. (2) With those five tests quarantined a **third** crash appeared with no timeout at all: 5.5 minutes of silence after the last SSU2 test, then a hard process death, with `ScaledNetworkFixture` never logging a line — so it is in fixture teardown or the namespace transition, not in a test. Either way all 11 ScaledNetwork tests are erased. Batch 3-2's `MIN_INTEGRATION_TESTS` guard detects the truncation correctly; detection is not the same as not losing the coverage. | A hanging test fails alone; the run reaches ScaledNetwork |
| 4-1a | `p4/ssu2-data-packet-header` | **Found while scoping 4-1; verified. The data phase has its own self-agreeing convention error, the same shape as 4-0b but unreached because SSU2 has never had a data phase.** (1) `SSU2DataPacket.BuildWithBlock` (`SSU2DataPacket.cs:60-82`) takes `dataKey` and `headerKey2`, **uses neither**, and returns the raw block list — so everything `SendBlock` has ever sent (relay tag requests, peer tests) went out unframed and unencrypted. (2) `BuildEncryptedPacket` (`:100-130`) writes a header that is not an SSU2 short header: 2 bytes of connection ID at 0-1, packet number at **4** not 8, and a `Type` byte at **2** while every reader takes type from **12**. `Parse` mirrors the same wrong layout, so build/parse round-trips and no test notices. (3) `ProcessReceivedPacket:621` hands a whole-packet clone to `DecryptShortHeader`, which throws for anything but exactly 16 bytes — so every data packet is dropped into the outer catch. Verifiable with no handshake, so it can land before 4-0b and 4-2. | Build/parse against the real short-header layout; a data packet dispatches to `ProcessDataPacket` |
