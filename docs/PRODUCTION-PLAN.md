...
| 4-0f | `p4/cancelafter-observes-its-token` | **Found by 4-0e, root cause identified.** `[CancelAfter(n)]` cancels a `CancellationToken` that NUnit passes **as a test-method parameter** — and not one of the 23 tests carrying the attribute declares one. So the timeout marks the test failed while its thread runs on untouched, and the orphans eventually take the test host down: *"The active test run was aborted. Reason: Test host process crashed"*. That erases every test scheduled after it — both of 4-0e's CI runs lost all 11 ScaledNetwork tests, to a different hanging test each time. Give the timed-out tests a `CancellationToken` parameter and thread it through `SAMHelper`, so a hang fails one test instead of the run. Consider also splitting the namespaces into separate `dotnet test` invocations for defence in depth. Batch 3-2's `MIN_INTEGRATION_TESTS` guard detects the truncation correctly; detection is not the same as not losing the coverage. | A test that hangs fails alone; the run completes |
