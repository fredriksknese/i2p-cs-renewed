...
| 4-0f | `p4/integration-namespace-isolation` | **Found by 4-0e.** One hanging test crashes the test host and erases every test scheduled after it — `TestSend5MB_ECIES_X25519` timing out cost all 11 ScaledNetwork tests, which run in a later namespace. 23 tests carry `CancelAfter`, so this recurs as Phase 5 makes more of them reach a real transfer. Run the namespaces as separate `dotnet test` invocations (or otherwise isolate them) so a crash truncates one group, not the run. Batch 3-2's `MIN_INTEGRATION_TESTS` guard catches it correctly today; the point is to stop losing the coverage. | Kill a test host mid-run; the other namespace still reports |
