...
namespace I2PTests.IntegrationTests
{
    ...
    public class DataTransferTests
    {
        ...
        private async Task WaitForTunnels( int timeoutMs = 120000 )
        {
            ...
            {
                var outCount = I2PCore.TunnelLayer.TunnelProvider.Inst?.OutboundTunnelCount ?? 0;
                var inCount = I2PCore.TunnelLayer.TunnelProvider.Inst?.InboundTunnelCount ?? 0;
        ...
        public async Task TestSend5MB_CSharpToI2pd_SAM()
        {
            ...
            var testData = SAMHelper.GenerateTestData( DataSize, seed: 100 );
        ...
        public async Task TestSend5MB_I2pdToCSharp_SAM()
        {
            ...
            var testData = SAMHelper.GenerateTestData( DataSize, seed: 200 );
            var expectedHash = SAMHelper.ComputeSha256( testData );
        ...
        public async Task TestBidirectional5MB_SAM()
        {
            ...
            var dataA = SAMHelper.GenerateTestData( DataSize, seed: 300 );
            var dataB = SAMHelper.GenerateTestData( DataSize, seed: 400 );
            ...
            using var acceptA = await SAMHelper.CreateAndHelloAsync(
                "127.0.0.1", PortAllocator.WellKnown.CSharpSam );
            var acceptTaskA = Task.Run( async () =>
            {
                await acceptA.StreamAcceptAsync( "bidir_a" );
            } );
        ...
        public async Task TestSend5MB_ECIES_X25519()
        {
            ...
            var testData = SAMHelper.GenerateTestData( DataSize, seed: 500 );
            var expectedHash = SAMHelper.ComputeSha256( testData );
        ...
        public async Task TestSend5MB_MLKEM768_Hybrid()
        {
            ...
            var testData = SAMHelper.GenerateTestData( DataSize, seed: 600 );
            var expectedHash = SAMHelper.ComputeSha256( testData );
