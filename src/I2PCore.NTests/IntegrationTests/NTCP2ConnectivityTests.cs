...
namespace I2PTests.IntegrationTests
{
    ...
    public class NTCP2ConnectivityTests
    {
        ...
        public async Task TestCSharpConnectsToI2pd_NTCP2()
        {
            ...
            var ntcp2Addr = i2pdInfo.Addresses?.FirstOrDefault(
                a => a.TransportStyle?.ToString() == "NTCP2"
            ...
            var connected = await router.WaitForConnection( i2pdHash, timeoutMs: 30000 );
        ...
        public async Task TestI2pdConnectsToCSharp_NTCP2()
        {
            ...
            var connected = await router.WaitForConnection( i2pdHash, timeoutMs: 60000 );

            if ( !connected )
            {
                // Try triggering the connection by sending to i2pd first
                router.ConnectToPeer( TestNetworkFixture.I2pdRouterInfo );
                connected = await router.WaitForConnection( i2pdHash, timeoutMs: 30000 );
            }
        ...
        public async Task TestNTCP2DataExchange_I2pd()
        {
            ...
            var dsm = new I2PCore.TunnelLayer.I2NP.Messages.DatabaseStoreMessage(
                I2PCore.SessionLayer.RouterContext.Inst.MyRouterInfo );
        ...
        public async Task TestNTCP2BlockProcessing()
        {
            ...
            var storedRi = I2PCore.NetDb.Inst[i2pdHash];
            if ( storedRi == null )
            {
                var routers = I2PCore.NetDb.Inst.GetRouters().Select( ih => ih.Id32Short ).ToArray();
                var debugInfo = I2PCore.NetDb.Inst.GetRouterInfoDebug( i2pdHash );
                Logging.LogInformation( $"[DEBUG_LOG] TestNTCP2BlockProcessing: i2pdHash {i2pdHash} not found in NetDb. Debug: {debugInfo}. Known routers: {string.Join( ", ", routers )}" );
            }
        ...
        public async Task TestCSharpConnectsToI2pd_NTCP2_PQ()
        {
            ...
            var hasPQ = i2pdInfo.Addresses?.Any(
                a => a.Options.Any( o =>
                    o.Key.ToString() == "i" &&
