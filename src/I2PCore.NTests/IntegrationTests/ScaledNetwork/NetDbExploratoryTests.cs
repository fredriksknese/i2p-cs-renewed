...
public class NetDbExploratoryTests
{
    ...
    {
        var inboundOk = false;
        var outboundOk = false;

        for (var i = 0; i < TunnelWaitIterations; ++i)
        {
    ...
    public async Task ClientTunnelsAndLeaseSetPublication()
    {
        ...
        var csSamPort = PortAllocator.Scaled.Cs0Sam;
        var i2pdSamPort = PortAllocator.Scaled.I2pd1Sam;
        ...
        var receiverTask = Task.Run(async () =>
        {
            ...
            var read = await stream.ReadAsync(buf, 0, buf.Length, readCts.Token);
            var msg = Encoding.UTF8.GetString(buf, 0, read);
            Logging.LogInformation($"C# Receiver got message: {msg}");

            var ackMsg = Encoding.UTF8.GetBytes("ACK: " + msg);
        ...
        for (var i = 0; i < 60; ++i)
        {
            ...
            var bridge = ClientContext.Inst.SAMBridge;
            if (bridge != null && bridge.TryGetSession("CS_DEST", out var session))
            {
                var dest = session.Destination;
                var inCount = dest.InboundTunnelCount;
                var outCount = dest.OutboundTunnelCount;
        ...
        await i2pdSam.CreateSessionAsync("I2PD_CLIENT",
            timeoutMs: 300_000);
        ...
        for (var i = 0; i < 40; ++i)
        {
            ...
            try
            {
                ...
                await i2pdConn.SendDataAsync(Encoding.UTF8.GetBytes(testMsg), cts.Token);
    ...
    public async Task LeaseSetLookupAndEncryptionVerification()
    {
        ...
        var samPort = PortAllocator.Scaled.I2pd0Sam;
    ...
    public async Task SamDataTransferWithLeaseSetLookup()
    {
        ...
        var testData = SAMHelper.GenerateTestData(64 * 1024, 9999); // 64KB
