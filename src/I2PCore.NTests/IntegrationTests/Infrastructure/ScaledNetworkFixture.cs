...
namespace I2PTests.ScaledNetwork
{
    ...
    public class ScaledNetworkFixture
    {
        ...
        private async Task StartScaledNetwork()
        {
            ...
            if ( I2PTests.IntegrationTests.TestNetworkFixture.CSharpRouter != null )
            {
                CSharp0 = I2PTests.IntegrationTests.TestNetworkFixture.CSharpRouter;
            ...
            else
            {
                CSharp0 = new CSharpRouterHarness(
                    ntcp2Port: PortAllocator.Scaled.Cs0Ntcp2,
                    ssu2Port: PortAllocator.Scaled.Cs0Ssu2,
                    samPort: PortAllocator.Scaled.Cs0Sam,
                    i2cpPort: PortAllocator.Scaled.Cs0I2cp,
                    floodfill: true );
                CSharp0.Start();
            ...
            for ( int i = 2; i <= 8; i++ )
            {
                i2pdConfigs.Add( CreateI2pdConfig( i, false, allKnownInfos ) );
            }
            ...
            var allInfos = new[] { Cs0Info, I2pd0Info, I2pd1Info, I2pd2Info, I2pd3Info, I2pd4Info, I2pd5Info, I2pd6Info, I2pd7Info, I2pd8Info }
            ...
            for ( int i = 0; i < allI2pdConfigs.Length; i++ )
            {
        ...
        public static (string dataDir, string configPath) CreateI2pdConfig(
            int index, bool floodfill, I2PCore.Data.I2PRouterInfo[] peerInfos )
        {
            ...
            var configContent = I2pdConfigGenerator.GenerateConfig(
                dataDir,
                ntcp2Port: ports.Item1,
                ssu2Port: ports.Item2,
                samPort: ports.Item3,
                i2cpPort: ports.Item4,
                httpPort: ports.Item5,
                floodfill: floodfill,
        ...
        private static void LogRouterInfo( string label, I2PCore.Data.I2PRouterInfo ri )
        {
            if ( ri != null )
                Logging.LogInformation(
        ...
        {
            try { I2pd8?.Dispose(); } catch { }
            try { I2pd7?.Dispose(); } catch { }
            try { I2pd6?.Dispose(); } catch { }
            try { I2pd5?.Dispose(); } catch { }
            try { I2pd4?.Dispose(); } catch { }
            try { I2pd3?.Dispose(); } catch { }
            try { I2pd2?.Dispose(); } catch { }
            try { I2pd1?.Dispose(); } catch { }
            try { I2pd0?.Dispose(); } catch { }
            if ( _ownsCSharp0 ) try { CSharp0?.Dispose(); } catch { }
