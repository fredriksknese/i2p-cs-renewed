...
namespace I2PTests.IntegrationTests.MultiHop
{
    ...
    public class MultiHopTestFixture
    {
        ...
        private async Task StartFourRouterNetwork()
        {
            ...
            else
            {
                ...
                var i2pdAConfig = I2pdConfigGenerator.GenerateConfig(
                    I2pdADataDir,
                    ntcp2Port: PortAllocator.WellKnown.I2pdNtcp2,
                    ssu2Port: PortAllocator.WellKnown.I2pdSsu2,
                    samPort: PortAllocator.WellKnown.I2pdSam,
                    i2cpPort: PortAllocator.WellKnown.I2pdI2cp,
                    httpPort: PortAllocator.WellKnown.I2pdHttp );
                ...
                await I2pdAManager.WaitForReady( PortAllocator.WellKnown.I2pdNtcp2, 60000 );
            ...
            await CSharpB.Start();
            await CSharpB.WaitForReady( timeoutMs: 60000 );

            // Get C# B's RouterInfo
            try
            {
                CSharpBInfo = await CSharpB.WaitForRouterInfo( timeoutMs: 30000 );
                Logging.LogInformation(
            ...
            var i2pdBConfig = I2pdConfigGenerator.GenerateConfig(
                I2pdBDataDir,
                ntcp2Port: PortAllocator.WellKnown.I2pdBNtcp2,
                ssu2Port: PortAllocator.WellKnown.I2pdBSsu2,
                samPort: PortAllocator.WellKnown.I2pdBSam,
                i2cpPort: PortAllocator.WellKnown.I2pdBI2cp,
                httpPort: PortAllocator.WellKnown.I2pdBHttp );
            ...
            await I2pdBManager.WaitForReady( PortAllocator.WellKnown.I2pdBNtcp2, 60000 );
        ...
        {
            try { I2pdBManager?.Dispose(); } catch { }
            // Only dispose i2pd A if we started it (not reused from TestNetworkFixture)
            if ( _ownsI2pdA ) try { I2pdAManager?.Dispose(); } catch { }
            try { CSharpB?.Dispose(); } catch { }
            // Only dispose C# A if we started it (not reused from TestNetworkFixture)
            if ( _ownsCSharpA ) try { CSharpA?.Dispose(); } catch { }
