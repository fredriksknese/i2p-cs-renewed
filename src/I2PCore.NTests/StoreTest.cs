...
public class StoreTest
{
    ...
    public void WriteTest1()
    {
        ...
        var defaultsectorsize = 100;
        while (defaultsectorsize < 32000)
        {
            using (var target = new Store(filename, defaultsectorsize))
            {
    ...
    public void WriteTest()
    {
        ...
        var defaultsectorsize = 30;
        while (defaultsectorsize < 32000)
        {
            using (Stream dest = new MemoryStream())
            {
                using (var target = new Store(dest, defaultsectorsize))
                {
    ...
    {
        var defaultsectorsize = 1024;
        using (Stream dest = new MemoryStream())
        {
            using (var target = new Store(dest, defaultsectorsize))
            {
                for (var i = 0; i < 1000; ++i)
                    target.Write(new[] { (byte)BufUtils.RandomInt(240), (byte)(i % 5), (byte)BufUtils.RandomInt(240) });
    ...
    public void ReadAllTest()
    {
        ...
        using (Stream dest = new MemoryStream())
        {
            using (var target = new Store(dest, defaultsectorsize))
            {
                ...
                for (var i = 0; i < 1000; ++i) Assert.IsTrue(allofit[i].Value[1] == i % 5);
            }
        }
    }
    ...
    {
        var defaultsectorsize = 60;
        using (Stream dest = new MemoryStream())
        {
            using (var target = new Store(dest, defaultsectorsize))
            {
                for (var i = 0; i < 1000; ++i)
                    target.Write(new[] { (byte)BufUtils.RandomInt(240), (byte)(i % 7), (byte)BufUtils.RandomInt(240) });
    ...
    public void IndexStreamTest3()
    {
        ...
        while (defaultsectorsize < maxSectorSize)
        {
            using (Stream dest = new MemoryStream())
            {
                using (var mid = new Store(dest, maxSectorSize - defaultsectorsize + BufUtils.RandomInt(15)))
                {
                    mid.IndexStream();
                    mid.IndexStream();
                    mid.IndexStream();

                    using (var target = new Store(mid.IndexStream(), defaultsectorsize))
                    {
                        var firstsector = new byte[target.FirstSectorDataSize];
                        firstsector.Populate<byte>(1);
                        ...
                        using (var innerstore = new Store(target.IndexStream(),
                                   defaultsectorsize * 2 + BufUtils.RandomInt(12)))
                        {
                            var ix = innerstore.Write(data1);
                            Assert.AreEqual(data1.Length, innerstore.GetDataLength(ix));
                            var copy = innerstore.Read(ix);
                        ...
                        Assert.IsTrue(
                            BufUtils.Equal(target.Read(strix).Take(firstsector.Length).ToArray(), firstsector));
                    }
                }
            }
    ...
    public void DeleteTest1()
    {
        ...
        using (Stream dest = new MemoryStream())
        {
            using (var target = new Store(dest, defaultsectorsize))
            {
                ...
                for (var i = 0; i < 1000; ++i) target.Write((i & 1) != 0 ? data1 : data);
