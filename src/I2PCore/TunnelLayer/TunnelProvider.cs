...
public enum TunnelPoolSelection
{
    RejectExploratory,
    AllowExploratory,
    RequireExploratory
...
public class TunnelProvider
{
    ...
    private static readonly ConcurrentDictionary<BuildResponseRecord.RequestResponse, TunnelBuildRepliesInfo>
        _tunnelBuildReplies =
            new();
    ...
    private readonly ConcurrentQueue<(Ii2NpHeader msg, I2PIdentHash transportFrom)> IncomingMessageQueue = new();

    private readonly AutoResetEvent IncommingMessageReceived = new(false);

    /// <summary>
    ///     Registry of garlic tags for tunnel build reply decryption.
    ///     Key: 8-byte garlic tag, Value: (garlicKey, outboundTunnel)
    /// </summary>
    private readonly ConcurrentDictionary<ulong, (byte[] Key, Tunnel Tunnel)> PendingGarlicTags = new();

    // The byte value have no real meaning, the dictionaries are hash sets.
    private readonly ConcurrentDictionary<InboundTunnel, byte> PendingInbound = new();
    private readonly TickSpan PendingLookupTimeout = TickSpan.Seconds(20);
    private readonly ConcurrentDictionary<OutboundTunnel, byte> PendingOutbound = new();

    private readonly ConcurrentDictionary<I2PIdentHash, ConcurrentQueue<PendingShortRequest>> PendingShortLookups =
        new();

    private readonly PeriodicAction QueueStatusLog = new(TickSpan.Seconds(30));

#if DEBUG
    private readonly TimeWindowDictionary<uint, RefPair<TickCounter, int>> ReallyOldTunnelBuilds =
        new(TickSpan.Minutes(10));

#endif
    private readonly PeriodicAction TunnelBandwidthLog = new(TickSpan.Minutes(8));
    ...
    private TunnelProvider()
    {
        ...
        TransportProvider.Inst.IncomingMessage += DistributeIncomingMessage;
        NetDb.Inst.RouterInfoUpdates += NetDb_RouterInfoUpdates;
    }

    public static TunnelProvider Inst { get; protected set; }

    public int InboundTunnelCount => EstablishedInbound.Count;
    public int OutboundTunnelCount => EstablishedOutbound.Count;

    internal bool ClientTunnelsStatusOk { get; set; }
    internal bool AcceptTransitTunnels => ClientTunnelsStatusOk || !RouterContext.Inst.IsFirewalled;
    ...
    public void RegisterBuildReplyGarlicTag(ulong tag, byte[] key, Tunnel tunnel)
    {
        ...
        try
        {
            ...
            var tagObj = new SessionTag(tagBytes);
            Router.EciesRouterProcessor?.SessionManager?.RegisterOneTimeSession(tagObj, key);
        }
        catch (Exception ex)
        {
    ...
    /// </summary>
    public bool TryHandleBuildReplyGarlic(GarlicMessage garlicmsg, InboundTunnel from)
    {
        ...
        Logging.LogInformation(
            $"TunnelProvider: TryHandleBuildReplyGarlic tag={tag:X16}, data={data.Length} bytes, rawTagLE={tagValue:X16}");
        ...
        if (!PendingGarlicTags.TryRemove(tag, out var entry))
        {
            if (PendingGarlicTags.Count > 0)
            {
                ...
                Logging.LogInformation(
                    $"TunnelProvider: TryHandleBuildReplyGarlic tag={tag:X16} NOT FOUND. Pending Tags: {allTags}");
            }
        ...
        Logging.LogInformation(
            $"TunnelProvider: Matched garlic tag {tag:X16} for {tunnelType} tunnel build reply {entry.Tunnel.TunnelDebugTrace}");
        ...
        while (offset + 3 <= decrypted.Length)
        {
            ...
            if (blockType == 11)
            {
                ...
                try
                {
                    ...
                    Logging.LogInformation(
                        $"TunnelProvider: Garlic clove: type={msgType}, ID={msgId:X8}, expiration={expirationDate}");
                    ...
                    if (cloveBuf.Remaining > 0)
                    {
                        ...
                        var copy = new byte[payloadData.Length + I2NpMessage.I2NpMaxHeaderSize];
                        Array.Copy(payloadData, 0, copy, I2NpMessage.I2NpMaxHeaderSize, payloadData.Length);

                        var msg = I2NpUtil.GetMessage(msgType, new I2PBufferCursor(copy, I2NpMessage.I2NpMaxHeaderSize),
                            msgId);
                        if (msg != null)
                        {
                            msg.Expiration = expirationDate;
                            Logging.LogInformation(
                                $"TunnelProvider: Extracted {msgType} from garlic build reply. Buffer size: {copy.Length}");
    ...
    private void NetDb_RouterInfoUpdates(I2PRouterInfo ri)
    {
        ...
        {
            Logging.LogInformation(
                $"TunnelProvider: RouterInfo for {ri.Identity.IdentHash.Id32Short} found. Resuming {queue.Count} pending ECIES build requests.");
            while (queue.TryDequeue(out var request))
                if (request.Created.DeltaToNow < PendingLookupTimeout)
                    ...
                    Logging.LogDebug(
                        $"TunnelProvider: Pending ECIES build request for {ri.Identity.IdentHash.Id32Short} expired.");
        }
    }
    ...
    private void CleanupPendingShortLookups()
    {
        foreach (var key in PendingShortLookups.Keys.ToArray())
            if (PendingShortLookups.TryGetValue(key, out var queue))
            {
                ...
                if (allExpired) PendingShortLookups.TryRemove(key, out _);
            }
    }
    ...
    private static void RunNoiseNSelfTest()
    {
        try
        {
            ...
            var encPubKey = ourPubKey.Length == 32
                ? ourPubKey
                : ourPubKey.Skip(ourPubKey.Length - 32).Take(32).ToArray();

            Logging.LogCritical(
                $"[SELF-TEST] NoiseN: Identity pubkey ({encPubKey.Length}b): {BitConverter.ToString(encPubKey, 0, 8)}...");

            // Create a test build request record
            var testRecord = new ShortBuildRequestRecord
            {
            ...
            Logging.LogCritical(
                $"[SELF-TEST] NoiseN: Cleartext ({cleartext.Length}b): {BitConverter.ToString(cleartext, 0, Math.Min(16, cleartext.Length))}...");
            ...
            Logging.LogCritical(
                $"[SELF-TEST] NoiseN: Encrypted ({noiseMessage.Length}b): ephemeral={BitConverter.ToString(noiseMessage, 0, 8)}...");
            Logging.LogCritical(
                $"[SELF-TEST] NoiseN: Initiator CK={BitConverter.ToString(initCK, 0, 8)}..., Hash={BitConverter.ToString(initHash, 0, 8)}...");
            ...
            var decPrivKey = ourPrivKey.Length == 32
                ? ourPrivKey
                : ourPrivKey.Skip(ourPrivKey.Length - 32).Take(32).ToArray();
            ...
            Logging.LogCritical(
                $"[SELF-TEST] NoiseN: Decrypted ({decrypted.Length}b): {BitConverter.ToString(decrypted, 0, Math.Min(16, decrypted.Length))}...");
            Logging.LogCritical(
                $"[SELF-TEST] NoiseN: Responder CK={BitConverter.ToString(respCK, 0, 8)}..., Hash={BitConverter.ToString(respHash, 0, 8)}...");

            // Verify cleartext matches
            var cleartextMatch = cleartext.SequenceEqual(decrypted);
            var ckMatch = initCK.SequenceEqual(respCK);
            var hashMatch = initHash.SequenceEqual(respHash);

            Logging.LogCritical(
                $"[SELF-TEST] NoiseN: Cleartext match={cleartextMatch}, CK match={ckMatch}, Hash match={hashMatch}");
            ...
            var layerMatch = initLayer.SequenceEqual(respLayer);
            var ivMatch = initIv.SequenceEqual(respIv);
            var replyMatch = initReply.SequenceEqual(respReply);
            var garlicMatch = initGarlic != null && respGarlic != null && initGarlic.SequenceEqual(respGarlic);
            var tagMatch = initTag == respTag;

            Logging.LogCritical(
                $"[SELF-TEST] NoiseN: Key derivation: layer={layerMatch}, iv={ivMatch}, reply={replyMatch}, garlic={garlicMatch}, tag={tagMatch}");
            ...
            var stbm = new ShortTunnelBuildMessage(new List<byte[]>
            {
                onWireRecord,
            ...
            var bytesPreserved = onWireRecord.SequenceEqual(stbmRecordBytes);
            ...
            if (!bytesPreserved)
                ...
                for (var d = 0; d < Math.Min(onWireRecord.Length, stbmRecordBytes.Length); ++d)
                    if (onWireRecord[d] != stbmRecordBytes[d])
                    {
                        Logging.LogCritical(
                            $"[SELF-TEST] NoiseN: First diff at offset {d}: orig=0x{onWireRecord[d]:X2}, stbm=0x{stbmRecordBytes[d]:X2}");
                        break;
                    }
            ...
            var noisePreserved = noiseMessage.SequenceEqual(extractedNoise);
            ...
            try
            {
                ...
                noiseResp2.Dispose();
                Logging.LogCritical(
                    $"[SELF-TEST] NoiseN: Direct decrypt of extracted bytes: OK ({decrypted2.Length}b)");
            }
            catch (Exception ex2)
            {
            ...
            var peekPreserved = noiseMessage.SequenceEqual(peekNoise);
            Logging.LogCritical($"[SELF-TEST] NoiseN: Peek-extracted noise preserved={peekPreserved}");

            if (!peekPreserved)
                Logging.LogCritical(
                    $"[SELF-TEST] NoiseN: peekNoise[0:8]={BitConverter.ToString(peekNoise, 0, 8)}, expected={BitConverter.ToString(noiseMessage, 0, 8)}");

            var decrypt = new ECIESTunnelDecrypt(decPrivKey, encPubKey);
            var result = decrypt.ProcessShortTunnelBuild(stbm);

            Logging.LogCritical(
                $"[SELF-TEST] NoiseN: Full pipeline: success={result.Success}, recordIdx={result.RecordIndex}");
            ...
            if (result.Success)
            {
                ...
                var recvMatch = (uint)parsed.ReceiveTunnelId == 12345u;
                var nextMatch = parsed.NextRouterHash?.Equals(ourIdentHash) ?? false;
                var flagsMatch = parsed.Flags == (byte)ShortBuildRequestRecord.BuildRequestFlags.OutboundEndpoint;
                var msgIdMatch = parsed.NextMessageId == 0xDEADBEEF;

                Logging.LogCritical(
                    $"[SELF-TEST] NoiseN: Field verification: recv={recvMatch}, next={nextMatch}, flags={flagsMatch}, msgId={msgIdMatch}");

                if (recvMatch && nextMatch && flagsMatch && msgIdMatch)
                    Logging.LogCritical(
                        "[SELF-TEST] NoiseN: ALL TESTS PASSED - Noise N crypto is internally consistent");
                else
        ...
        try
        {
            ...
            while (!Terminated)
                try
                {
                    ...
                    ExecuteQueue(
                        PendingOutbound.Select(d => d.Key), t => PendingOutbound.TryRemove((OutboundTunnel)t, out _),
                        true);

                    ExecuteQueue(
                        PendingInbound.Select(d => d.Key), t => PendingInbound.TryRemove((InboundTunnel)t, out _),
                        true);
    ...
    {
        if (PendingOutbound.Count > 0) CheckForTunnelBuildTimeout(PendingOutbound.Keys.ToArray());

        if (PendingInbound.Count > 0) CheckForTunnelBuildTimeout(PendingInbound.Keys.ToArray());
    }
    ...
    private void CheckForTunnelBuildTimeout(IEnumerable<Tunnel> pool)
    {
        ...
        {
            TunnelBuildLogger.Inst.Log($"Tunnel build timeout: {one.TunnelDebugTrace}", one.TunnelDebugTrace,
                one.Pool.ToString(), one.TunnelDirection.ToString());
#if DEBUG
        ...
        if (removedtunnels.Any())
        {
            ...
            foreach (var onepool in pools)
            {
                ...
                foreach (var one in pool) st.Append($"{one.TunnelDebugTrace} ");
            }
        ...
        while (FailedTunnels.TryDequeue(out var t))
        {
            ...
            if (isfailed)
            {
                if (ispending)
                {
                    ...
                    TunnelBuildLogger.Inst.Log($"Tunnel failed during build: {tunnel.TunnelDebugTrace}",
                        tunnel.TunnelDebugTrace, tunnel.Pool.ToString(), tunnel.TunnelDirection.ToString());
            ...
            {
                Logging.LogDebug(
                    $"TunnelProvider: ExecuteQueue removing expired tunnel {tunnel} created {tunnel.CreationTime}.");
    ...
    public Tunnel CreateTunnel(ITunnelOwner owner, TunnelConfig config,
        TunnelPoolSelection? replyTunnelPoolOverride = null)
    {
        ...
        if (config.Direction == TunnelConfig.TunnelDirection.Outbound)
        {
            ...
            Logging.LogDebug(
                $"TunnelProvider: Using reply tunnel: {replytunnel}, GatewayTunnelId={replytunnel.GatewayTunnelId}, ReceiveTunnelId={replytunnel.ReceiveTunnelId}");
            ...
            Logging.LogDebug(
                $"TunnelProvider: {tunnel} created, build id: {tunnel.TunnelBuildReplyMessageId} {req.MessageId} {req.CreateHeader16.MessageId}.");
            TunnelBuildLogger.Inst.Log(
                $"Creating outbound tunnel {tunnel.TunnelDebugTrace}: {TunnelBuildLogger.GetHopsString(config.Info.Hops)} via {tunnel.Destination.Id32Short} using reply tunnel {replytunnel.TunnelDebugTrace}",
                tunnel.TunnelDebugTrace, tunnel.Pool.ToString(), tunnel.TunnelDirection.ToString());

#if DEBUG
            ReallyOldTunnelBuilds.Set(tunnel.TunnelBuildReplyMessageId,
                new RefPair<TickCounter, int>(TickCounter.Now,
                    replytunnel.Config.Info.Hops.Count + config.Info.Hops.Count));
#endif
            ...
            if (endpointHop.GarlicKey != null && endpointHop.GarlicTag != 0)
            {
                ...
                Logging.LogInformation(
                    $"TunnelProvider: OB build {tunnel.TunnelDebugTrace}: dest={tunnel.Destination.Id32Short}, " +
        ...
        else
        {
            ...
            TunnelBuildLogger.Inst.Log(
                $"Creating inbound tunnel {tunnel.TunnelDebugTrace}: {TunnelBuildLogger.GetHopsString(config.Info.Hops)} via gateway {tunnel.Destination.Id32Short}",
                tunnel.TunnelDebugTrace, tunnel.Pool.ToString(), tunnel.TunnelDirection.ToString());
#if DEBUG
            ReallyOldTunnelBuilds.Set(tunnel.TunnelBuildReplyMessageId,
                new RefPair<TickCounter, int>(TickCounter.Now,
                    (outtunnel?.Config.Info.Hops.Count ?? 0) + config.Info.Hops.Count));
#endif
            ...
            if (outtunnel != null)
            {
                ...
                if (x25519pk != null)
                {
                    var ecies = Router.EciesRouterProcessor;
                    if (ecies != null)
                        try
                        {
                            // Decrypted payload: Block format containing a GarlicCloveBlock
                            var cloveStream = new ArrayBufferWriter<byte>();
                            cloveStream.WriteByte(0); // Delivery instructions: Local
                            ...
                            var expirationSeconds =
                                (uint)Math.Round(((DateTime)tunnelbuild.Expiration - I2PDate.RefDate).TotalSeconds);
                            ...
                            Logging.LogDebug(
                                $"TunnelProvider: Garlic wrapped inbound build request for {tunnel.TunnelDebugTrace} to gateway {tunnel.Destination.Id32Short}");
                        }
                        catch (Exception ex)
                        {
            ...
            {
                Logging.LogInformation(
                    $"TunnelProvider: No established outbound tunnels available for inbound tunnel {tunnel}. Sending directly to gateway {tunnel.Destination}.");
    ...
    private InboundTunnel AddZeroHopTunnel()
    {
        ...
        TunnelIds.Add(tunnel.ReceiveTunnelId, tunnel);
        Logging.LogDebug(
            $"TunnelProvider: Registered inbound tunnel ReceiveTunnelId={tunnel.ReceiveTunnelId}, GatewayTunnelId={tunnel.GatewayTunnelId}, established={tunnel.Established}");
        return tunnel;
    }
    ...
    internal void HandleIncomingMessage(Ii2NpHeader msg, InboundTunnel from = null, I2PIdentHash transportFrom = null)
    {
        if (msg.MessageType == I2NpMessage.MessageTypes.Garlic)
            Logging.LogInformation(
                $"TunnelProvider: Received Garlic message from {from?.TunnelDebugTrace ?? "DIRECT"}");
            ...
            || msg.MessageType == I2NpMessage.MessageTypes.Garlic)
        {
            ...
            {
                Logging.LogDebug(
                    $"TunnelProvider: Duplicate I2NP message dropped: {msg.MessageType} id={msg.Message?.MessageId}");
                return;
            }
        }
        ...
        switch (msg.MessageType)
        {
            ...
            case I2NpMessage.MessageTypes.TunnelGateway:
                ...
                else if (tg.TunnelId == 0)
                {
                    ...
                    HandleIncomingMessage(innerMsg0);
                }
                else
                {
                    Logging.LogWarning(
                        $"TunnelProvider: TunnelGateway tunnel {tg.TunnelId} NOT FOUND. Registered tunnels: {string.Join(", ", TunnelIds.GetAllTunnelIds())}");
                }
            ...
            case I2NpMessage.MessageTypes.Garlic:
                ...
                if (!TryHandleBuildReplyGarlic(directGarlic, from)) I2NpMessageReceived?.Invoke(msg, from);
                break;
    ...
    public void DistributeIncomingMessage(ITransport transp, Ii2NpHeader msg)
    {
        if (msg.MessageType == I2NpMessage.MessageTypes.ShortTunnelBuildReply)
            Logging.LogInformation(
                $"[DEBUG_LOG] DistributeIncomingMessage: Received ShortTunnelBuildReply from {transp?.RemoteRouterIdentity?.IdentHash?.Id32Short ?? "Tunnel"}");
        if (msg.MessageType == I2NpMessage.MessageTypes.VariableTunnelBuildReply)
            Logging.LogInformation(
                $"[DEBUG_LOG] DistributeIncomingMessage: Received VariableTunnelBuildReply from {transp?.RemoteRouterIdentity?.IdentHash?.Id32Short ?? "Tunnel"}");
        // Global inbound bandwidth enforcement
        if (RouterContext.Inst.ShouldDropInbound())
        {
    ...
    private void HandleShortTunnelBuild(Ii2NpHeader msg, I2PIdentHash from)
    {
        ...
        {
            Logging.LogInformation(
                $"HandleShortTunnelBuild: Matched pending inbound tunnel {matchingInbound.TunnelDebugTrace} by msgId={stbm.MessageId:X8}");
    ...
    private void HandleShortTunnelBuildRecords(Ii2NpHeader msg, ShortTunnelBuildMessage stbm, I2PIdentHash from)
    {
        ...
        var decrypt = new ECIESTunnelDecrypt(privateKey, publicKey);
        var result = decrypt.ProcessShortTunnelBuild(stbm);

        if (!result.Success)
        {
            // Fallback to NTCP2 transport key in case some implementations encrypt to the transport key
            var ntcp2Private = TransportProvider.Inst?.GetNTCP2StaticPrivateKey();
            var ntcp2Public = TransportProvider.Inst?.GetNTCP2StaticPublicKey();
            if (ntcp2Private != null && ntcp2Public != null && ntcp2Private != privateKey)
            {
                ...
                decrypt = new ECIESTunnelDecrypt(ntcp2Private, ntcp2Public);
        ...
        Logging.LogInformation(
            $"HandleShortTunnelBuildRecords: Decrypted ECIES request: recv={request.ReceiveTunnelId}, " +
        ...
        {
            Logging.LogInformation(
                $"HandleShortTunnelBuildRecords: NextHop {request.NextRouterHash.Id32Short} not in NetDb. Initiating lookup.");
            var queue = PendingShortLookups.GetOrAdd(request.NextRouterHash,
                _ => new ConcurrentQueue<PendingShortRequest>());
        ...
        var replyStatus = Router.TransitTunnelMgr.AcceptingTunnels(request.NextRouterHash)
            ? ShortBuildReplyRecord.TunnelBuildReplyStatus.Accept
            : ShortBuildReplyRecord.TunnelBuildReplyStatus.RejectBandwidth;

        // Hidden mode: reject
        if (RouterContext.Inst.IsHidden)
            replyStatus = ShortBuildReplyRecord.TunnelBuildReplyStatus.RejectBandwidth;
        ...
        for (var i = 0; i < stbm.Records.Count; i++)
        {
            ...
            var parameters = new ParametersWithIV(
                new KeyParameter(replyKey), nonce);
            ...
            cipher.ProcessBytes(recordData.BaseArray, recordData.BaseArrayOffset, recordData.Length,
                recordData.BaseArray, recordData.BaseArrayOffset);
        }

        if (replyStatus == ShortBuildReplyRecord.TunnelBuildReplyStatus.Accept)
        {
            ...
            if (request.ReceiveTunnelId != null)
            {
                ...
                var brrec = new BuildRequestRecord();
            ...
            if (request.NextRouterHash != null)
                try
                {
                    ...
                    if (request.NextTunnelId != 0)
                    {
                        ...
                        Logging.LogDebug(
                            $"TunnelProvider: ECIES OBEP: Sent Garlic-wrapped build reply to IBGW {request.NextRouterHash.Id32Short}:{request.NextTunnelId}");
                    }
                    else
                    {
                        ...
                        Logging.LogDebug(
                            $"TunnelProvider: ECIES OBEP: Sent Garlic-wrapped build reply directly to IBGW {request.NextRouterHash.Id32Short}");
                    }
                }
                catch (Exception ex)
                {
        ...
        else if (request.NextRouterHash != null)
        {
            ...
            try
            {
                ...
                Logging.LogDebug(
                    $"HandleShortTunnelBuildRecords: Forwarded to {request.NextRouterHash.Id32Short} msgId={request.NextMessageId:X8}");
            }
            catch (Exception ex)
            {
        ...
        TunnelBuildRequestDecrypt decrypt)
    {
        ...
        {
#if DEBUG
            ReallyOldTunnelBuilds.ProcessItem(decrypt.Decrypted.NextTunnel,
                (k, p) =>
                {
                    Logging.LogDebug(
                        $"Tunnel build req failed {decrypt.Decrypted.NextTunnel} age {p.Left.DeltaToNowMilliseconds / p.Right} msec / hop. Unknown tunnel id.");
                });
#endif
            return;
        }
        ...
        foreach (var tunnel in tunnels)
        {
            ...
            for (var i = setup.Hops.Count - 1; i >= 0; --i)
            {
    ...
    internal void HandleVariableTunnelBuildReply(VariableTunnelBuildReplyMessage msg)
    {
        ...
        if (!matchingOut.Any() && !matchingIn.Any())
            ReallyOldTunnelBuilds.ProcessItem(msg.MessageId, (k, p) =>
                Logging.LogDebug(
                    $"Tunnel build req failed {msg.MessageId} age {p.Left.DeltaToNowMilliseconds / p.Right} msec / hop. MessageId unknown.")
            );
#endif
    }
    ...
    {
        Console.WriteLine(
            $"[DEBUG_LOG] HandleShortTunnelBuildReply: Received reply MessageId={msg.MessageId:X8}, Records={msg.Records?.Count}");
        Logging.LogInformation(
            $"HandleShortTunnelBuildReply: Received reply MessageId={msg.MessageId:X8}, Records={msg.Records?.Count}");
        ...
        foreach (var obtunnel in matchingOut)
        {
            ...
            Logging.LogInformation(
                $"HandleShortTunnelBuildReply: MsgId match {msg.MessageId:X8} for outbound {obtunnel.TunnelDebugTrace}.");
            return;
        }
        ...
        foreach (var intunnel in matchingIn)
        {
            ...
            Logging.LogInformation(
                $"HandleShortTunnelBuildReply: MsgId match {msg.MessageId:X8} for inbound {intunnel.TunnelDebugTrace}.");
            return;
        }
        ...
        ReallyOldTunnelBuilds.ProcessItem(msg.MessageId, (k, p) =>
            Logging.LogDebug(
                $"ECIES Tunnel build req failed {msg.MessageId} age {p.Left.DeltaToNowMilliseconds / p.Right} msec / hop. MessageId unknown.")
        );
#endif
    }
    ...
    private bool HandleReceivedShortTunnelBuildReply(OutboundTunnel obtunnel, ShortTunnelBuildReplyMessage msg)
    {
        ...
        Logging.LogInformation(
            $"HandleReceivedShortTunnelBuildReply: Processing ECIES reply for {obtunnel.TunnelDebugTrace} with {hops.Count} hops, {msg.Records.Count} records");
        ...
        for (var p = 0; p < hops.Count; ++p)
        {
            ...
            for (var j = hops.Count - 1; j > p; --j)
            {
                var peelingHop = hops[j];
                if (peelingHop.ReplyKey != null) ChaCha20DecryptRecord(msg, recordIdx, peelingHop.ReplyKey);
            }
        }
        ...
        for (var h = 0; h < hops.Count; ++h)
        {
            ...
            if (hop.ReplyKey != null && hop.HandshakeHash != null)
            {
                ...
                {
                    Logging.LogWarning(
                        $"HandleReceivedShortTunnelBuildReply: [{h}] AEAD decryption failed for record {recordIdx}");
                    ok = false;
                }
            }
        }
        ...
        for (var i = 0; i < hops.Count; ++i)
        {
            var hop = hops[i];
            var recordIdx = hop.RecordIndex;
            if (recordIdx < 0 || recordIdx >= msg.Records.Count)
            {
                ok = false;
                continue;
            }
            ...
            var status = replyRecord.Length >= 202
                ? (ShortBuildReplyRecord.TunnelBuildReplyStatus)replyRecord[201]
                : ShortBuildReplyRecord.TunnelBuildReplyStatus.RejectBandwidth;

            Logging.LogInformation(
                $"HandleReceivedShortTunnelBuildReply: hop[{i}] record[{recordIdx}] from {hop.Peer.IdentHash.Id32Short} status={status}");
            Logging.LogInformation(
                $"[DEBUG_LOG] ShortTunnelBuildReply: hop[{i}] {hop.Peer.IdentHash.Id32Short} status={status}");
            TunnelBuildLogger.Inst.Log(
                $"ShortTunnelBuildReply: hop[{i}] {hop.Peer.IdentHash.Id32Short} status={status}",
                obtunnel.TunnelDebugTrace, obtunnel.Pool.ToString(), obtunnel.TunnelDirection.ToString());
    ...
    private bool HandleReceivedShortInboundTunnelBuildReply(InboundTunnel intunnel, ShortTunnelBuildReplyMessage msg)
    {
        ...
        Logging.LogInformation(
            $"HandleReceivedShortInboundTunnelBuildReply: Processing for {intunnel.TunnelDebugTrace} with {externalHopCount} external hops, {msg.Records.Count} records");

        // Step 1: Peel off ChaCha20 layers from each record individually.
        // Record p (owned by hop p) was encrypted by all hops j > p in reverse order.
        for (var p = 0; p < externalHopCount; ++p)
        {
            var targetHop = hops[p];
            var recordIdx = targetHop.RecordIndex;
            if (recordIdx < 0 || recordIdx >= msg.Records.Count) continue;
            ...
            for (var j = externalHopCount - 1; j > p; --j)
            {
                var peelingHop = hops[j];
                if (peelingHop.ReplyKey != null) ChaCha20DecryptRecord(msg, recordIdx, peelingHop.ReplyKey);
            }
        ...
        // Step 2: AEAD-decrypt each record using its hop's own keys.
        for (var h = 0; h < externalHopCount; ++h)
        {
            var hop = hops[h];
            var recordIdx = hop.RecordIndex;
            if (recordIdx < 0 || recordIdx >= msg.Records.Count) continue;
        ...
        for (var i = 0; i < externalHopCount; ++i)
        {
            var hop = hops[i];
            var recordIdx = hop.RecordIndex;
            if (recordIdx < 0 || recordIdx >= msg.Records.Count)
            ...
            Logging.LogInformation(
                $"HandleReceivedShortInboundTunnelBuildReply: hop[{i}] record[{recordIdx}] status={status}");
            TunnelBuildLogger.Inst.Log(
                $"ShortInboundTunnelBuildReply: hop[{i}] {hop.Peer.IdentHash.Id32Short} status={status}",
                intunnel.TunnelDebugTrace, intunnel.Pool.ToString(), intunnel.TunnelDirection.ToString());
    ...
    private static void ChaCha20DecryptRecord(ShortTunnelBuildReplyMessage msg, int recordIndex, byte[] replyKey)
    {
        ...
        cipher.ProcessBytes(recordData.BaseArray, recordData.BaseArrayOffset, recordData.Length, recordData.BaseArray,
            recordData.BaseArrayOffset);
    }
    ...
    private bool HandleReceivedInboundTunnelBuildReply(InboundTunnel intunnel, VariableTunnelBuildReplyMessage msg)
    {
        ...
        for (var i = extHopsCount - 1; i >= 0; --i)
        {
        ...
        for (var i = 0; i < extHopsCount; ++i)
        {
            ...
            {
                Logging.LogDebug(
                    $"InboundTunnel {intunnel.TunnelDebugTrace}: Inbound tunnel build reply, hash check failed from {hop.Peer.IdentHash.Id32Short}");
        ...
        if (ok)
        {
            ...
            TunnelBuildLogger.Inst.Log($"Inbound tunnel established (ElGamal): {intunnel.TunnelDebugTrace}",
                intunnel.TunnelDebugTrace, intunnel.Pool.ToString(), intunnel.TunnelDirection.ToString());
    ...
    private bool HandleReceivedTunnelBuildReply(OutboundTunnel obtunnel, VariableTunnelBuildReplyMessage msg)
    {
        ...
        for (var i = 0; i < hops.Count; ++i)
        {
            ...
            {
                Logging.LogDebug(
                    $"OutboundTunnel {obtunnel.TunnelDebugTrace}: Outbound tunnel build reply, hash check failed from {hop.Peer.IdentHash.Id32Short}");
                NetDb.Inst.Statistics.DestinationInformationFaulty(hop.Peer.IdentHash);
            }
        ...
        if (ok)
        {
            Inst.OutboundTunnelEstablished(obtunnel);
            foreach (var one in hops)
            {
                NetDb.Inst.Statistics.SuccessfulTunnelMember(one.Peer.IdentHash);
                RouterProfileManager.Instance.RecordTunnelBuild(one.Peer.IdentHash, true);
    ...
    private void OutboundTunnelEstablished(OutboundTunnel tunnel)
    {
        ...
        TunnelBuildLogger.Inst.Log($"Outbound tunnel established: {tunnel.TunnelDebugTrace}", tunnel.TunnelDebugTrace,
            tunnel.Pool.ToString(), tunnel.TunnelDirection.ToString());
            ...
            if (members != null)
            {
                ...
                try
                {
                    foreach (var member in members)
                        NetDb.Inst.Statistics.TunnelBuildTimeMsPerHop(member.IdentHash, deltaperhop);
                }
                catch (Exception ex)
                {
                    Logging.Log(ex);
    ...
    private void InboundTunnelEstablished(InboundTunnel tunnel)
    {
        tunnel.EstablishedTime.SetNow();
        tunnel.Established = true;
        TunnelBuildLogger.Inst.Log($"Inbound tunnel established: {tunnel.TunnelDebugTrace}", tunnel.TunnelDebugTrace,
            tunnel.Pool.ToString(), tunnel.TunnelDirection.ToString());
