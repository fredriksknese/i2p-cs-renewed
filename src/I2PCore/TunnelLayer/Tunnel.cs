...
public abstract class Tunnel
{
    ...
    public virtual bool NeedsRecreation
    {
        get
        {
            ...
            return CreationTime.DeltaToNow > Lifetime
                - (panic
    ...
    public virtual bool Expired =>
        EstablishedTime.DeltaToNow > Lifetime * 1.1 ||
        CreationTime.DeltaToNow > Lifetime * 1.2;

    public TunnelConfig.TunnelPool Pool => Config.Pool;
    public TunnelConfig.TunnelDirection TunnelDirection => Config.Direction;
    ...
    internal static TunnelInfo CreateOutboundTunnelChain(int hops, bool exploratory, bool bootstrapping = false)
    {
        ...
        if (connectedRouters.Count > 0 && hops >= 1 &&
            (exploratory || RouterContext.Inst.IsFirewalled || connectedRouters.Count < 15))
        {
            ...
            if (connectedList.Length > 0)
            {
                ...
                if (exploratory && remainingHops > 0)
                {
                    ...
                    for (var i = 0; i < remainingHops; ++i)
                    {
                        ...
                        var forceConnected = i == remainingHops - 1 &&
                                             (bootstrapping || RouterContext.Inst.IsFirewalled ||
                                              connectedRouters.Count < 10);
                ...
                else if (!exploratory && remainingHops >= 1)
                {
                    ...
                    if (middleHopsCount > 0)
                        foreach (var hop in NetDb.Inst.GetRandomRoutersForTunnelBuild(false, middleHopsCount,
                                     excludeSet))
                        {
                    ...
                    if (endpointRi != null) hopinfo.Add(new HopInfo(endpointRi.Identity, new I2PTunnelId()));
                }
            ...
            if (connectedList.Length > 0)
            {
                ...
                if (exploratory && remainingHops > 0)
                {
                    ...
                    for (var i = 0; i < remainingHops; ++i)
                    {
                        ...
                        var forceConnected = bootstrapping && connectedRouters.Count < 5;

                        if (forceConnected)
                        {
                            var choices = connectedList.Where(p => !excludeSet.Contains(p.Identity.IdentHash))
                                .ToArray();
                            if (choices.Length > 0) ri = choices[BufUtils.RandomInt(choices.Length)];
                        }
                ...
                {
                    foreach (var hop in NetDb.Inst.GetRandomRoutersForTunnelBuild(exploratory, remainingHops,
                                 excludeSet))
                    {
                        var ri = NetDb.Inst[hop];
                        if (ri?.Identity == null || excludeSet.Contains(ri.Identity.IdentHash)) continue;
    ...
    {
        var pool = Config?.Pool.ToString() ?? "<?>";
        return $"{GetType().Name} {pool} {TunnelDebugTrace} {CreationTime.DeltaToNow:MS}";
    }
}