...
public class TunnelPool : ITunnelOwner
{
    ...
    public TunnelPoolSettings Settings { get; }

    public int EstablishedCount => Tunnels.Count(t => t.Value && !t.Key.NeedsRecreation);

    public int RealEstablishedCount => Tunnels.Count(t =>
        t.Value && !t.Key.NeedsRecreation && !(t.Key is ZeroHopTunnel || t.Key is ZeroHopOutboundTunnel));
        ...
        for (var i = 0; i < count; ++i)
            try
            {
                ...
                if (tunnel == null)
                {
                    ++consecutiveFailures;
                    var msg =
                        $"Chain creation returned null ({this}, attempt {i + 1}/{count}). Connected: {TransportProvider.Inst.ConnectedRoutersCount}, NetDb: {NetDb.Inst.RouterCount}";
    ...
    {
        return
            $"TunnelPool {(Settings.IsInbound ? "Inbound" : "Outbound")} {(Settings.IsExploratory ? "Exploratory" : "Client")}";
    }
