...
public abstract partial class I2NpMessage
{
    ...
    private static readonly ItemFilterWindow<uint> _recentMessageIds = new(TickSpan.Minutes(10), 1);

    // Always allocated with space for a 16 byte header in front
    // Message payload starts at Payload.
    private I2PByteBlock Buf;

    private I2PDate ExpirationField;
    ...
    public virtual uint MessageId
    {
        get
        {
            if (MessageIdField.HasValue) return MessageIdField.Value;
    ...
    public I2PDate Expiration
    {
        get
        {
            if (ExpirationField != null) return ExpirationField;
    ...
    public I2PByteBlock Payload => Buf.Slice(I2NpMaxHeaderSize);

    public Ii2NpHeader16 CreateHeader16 => new I2NpHeader16(this);
    ...
    {
        Buf = new I2PByteBlock(start.BaseArray, start.BaseArrayOffset - I2NpMaxHeaderSize,
            reader.DistanceFrom(start) + I2NpMaxHeaderSize);
#if DEBUG
        HeaderState = HeaderStates.Invalid;
#endif
    }
    ...
    {
        return (T)I2NpUtil.GetMessage(src.MessageType,
            new I2PBufferCursor(src.Buf.Clone().ToByteArray(), I2NpMaxHeaderSize), src.MessageId);
    }
