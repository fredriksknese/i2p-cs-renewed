...
public class VariableTunnelBuildMessage : I2NpMessage
{
    ...
    private VariableTunnelBuildMessage(byte hops)
    {
        ...
        for (var i = 0; i < hops; ++i) Records.Add(new AesEgBuildRequestRecord(writer));
    }
    ...
    public override MessageTypes MessageType => MessageTypes.VariableTunnelBuild;
        ...
        uint replymessageid)
    {
        ...
        var allECIES = setup.Hops.All(hop => IsECIESRouter(hop.Peer));
        ...
        if (allECIES && setup.Hops.Count <= 8)
            try
            {
                ...
                Logging.LogInformation(
                    $"VariableTunnelBuildMessage: Building ECIES-only tunnel with {setup.Hops.Count} hops");
                return eciesMessage;
            }
            catch (Exception ex)
            {
                Logging.LogWarning(
                    $"VariableTunnelBuildMessage: ECIES tunnel build failed, falling back to ElGamal: {ex.Message}");
            }
        else if (allECIES)
        ...
        var usehops = (byte)(setup.Hops.Count > 5 ? 8 : 5);
        ...
        for (var i = 0; i < setup.Hops.Count; ++i)
        {
        ...
        // Scramble the rest
        for (var i = setup.Hops.Count; i < usehops; ++i) order[i].Data.Randomize();
        ...
        uint replymessageid)
    {
        ...
        if (allECIES && setup.Hops.Count <= 8)
            try
            {
                ...
                Logging.LogInformation(
                    $"VariableTunnelBuildMessage: Building ECIES-only INBOUND tunnel with {setup.Hops.Count} hops");
                return eciesMessage;
            }
            catch (Exception ex)
            {
                Logging.LogWarning(
                    $"VariableTunnelBuildMessage: ECIES inbound tunnel build failed, falling back: {ex.Message}");
            }
        ...
        for (var i = 0; i < setup.Hops.Count; ++i)
        {
            ...
            if (endpoint || i + 2 >= setup.Hops.Count) rec.SendMessageId = replymessageid;
        ...
        for (var i = setup.Hops.Count - 2; i >= 0; --i)
        {
            ...
            cipher.Init(false,
                new ParametersWithIV(new KeyParameter(prevhop.ReplyKey.ToByteArray()), prevhop.ReplyIv.ToByteArray()));
        ...
        uint replymessageid)
    {
        ...
        for (var i = 0; i < numHops; ++i)
        {
            ...
            Logging.LogCritical(
                $"[DEBUG_LOG] BuildECIESOutboundTunnel: hop={i}/{numHops} endpoint={endpoint} router={hop.Peer.IdentHash.Id32Short} identityPubKey({hopPubKey.Length}b)={BitConverter.ToString(hopPubKey, 0, Math.Min(8, hopPubKey.Length))}... ntcp2Key={ntcp2Hex}... hashPrefix={hashPrefix}...");
        ...
        for (var i = numHops; i < numRecords; ++i)
        ...
        uint replymessageid)
    {
        ...
        for (var i = 0; i < externalHopCount; ++i)
        {
            ...
            byte flags = 0;
            if (gateway) flags |= (byte)ShortBuildRequestRecord.BuildRequestFlags.InboundGateway;
        ...
        for (var i = externalHopCount; i < numRecords; ++i)
            allRecords[recordIndices[i]] = BufUtils.RandomBytes(ShortTunnelBuildMessage.RecordSize);
        ...
        int numRecords)
    {
        ...
        for (var h = realHopCount - 2; h >= 0; --h)
        {
            ...
            for (var i = 0; i < realHopCount; ++i) realIndices.Add(hops[i].RecordIndex);
