...
public class TunnelDataMessage : I2NpMessage
{
    ...
    public override MessageTypes MessageType => MessageTypes.TunnelData;

    public I2PTunnelId TunnelId
    {
        get => new(Payload.ReadUInt32BigEndian(0));
        set => Payload.WriteUInt32BigEndian(value, 0);
    }

    public I2PByteBlock Iv
    {
        get => Payload.Slice(4, 16);
        set => Payload.CopyFrom(value, 4);
    }

    public I2PByteBlock EncryptedWindow
    {
        get => Payload.Slice(20, 1008);
        set => Payload.CopyFrom(Payload, 20);
    }

    public I2PByteBlock Checksum
    {
        get => Payload.Slice(20, 4);
        set => Payload.CopyFrom(value, 20);
    }

    public I2PByteBlock PaddingStart
    {
        get => Payload.Slice(24, 1004);
        set => Payload.CopyFrom(value, 24);
    }

    public ushort DataLength => 1024;
    ...
    public static IEnumerable<TunnelDataMessage> MakeFragments(IEnumerable<TunnelMessage> messages,
        I2PTunnelId desttunnel)
    {
        ...
        foreach (var one in padcalc.TdMessages)
        {
            ...
            if (one.PaddingNeeded > 0) writer.WriteBytes(BufUtils.RandomNz(one.PaddingNeeded));

            writer.WriteByte(0); // The Zero
            one.TunnelDataInstance.SetFirstDeliveryInstructionPoint(writer.CurrentBlock);

            foreach (var frag in one.Fragments) frag.Append(writer);
    ...
    private static PaddingInfo CalculatePadding(IEnumerable<TunnelMessage> messages, I2PTunnelId desttunnel)
    {
        ...
        var tunneldatabufferavailable = FreeSpaceInDataMessageBody;
