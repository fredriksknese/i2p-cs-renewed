...
public class RouterProfile
{
    ...
    {
        var successRate = TunnelBuildAttempts > 0
            ? (double)TunnelBuildSuccesses / TunnelBuildAttempts
            : 0.5;

        var responseScore = AverageResponseTime > 0
            ? Math.Max(0, 1.0 - AverageResponseTime / 5000.0)
            : 0.5;

        // Penalize consecutive failures
        var failPenalty = Math.Max(0, 1.0 - ConsecutiveFailures * 0.2);

        // Penalize recent failures
        var recentFailPenalty = 1.0;
...
public class RouterProfileManager
{
    ...
    {
        foreach (var kvp in _profiles) kvp.Value.Save(profilesDirectory);
    }
    ...
    public void LoadAll(string profilesDirectory)
    {
        ...
        foreach (var file in Directory.GetFiles(profilesDirectory, "*.profile"))
        {
            ...
            if (profile != null) _profiles.TryAdd(profile.RouterHash, profile);
        }
    }
}