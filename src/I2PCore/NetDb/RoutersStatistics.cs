...
namespace I2PCore
{
    ...
    {
        private enum StoreRecordId : int { RouterStatistics = 1 };

        private ConcurrentDictionary<I2PIdentHash, RouterStatistics> Routers = new();
        ...
        {
            Update( hash, ds => ds.TunnelBuildTimeMsPerHop = ( ds.TunnelBuildTimeMsPerHop == 0 ? ms : ( long)( ( 9.0 * ds.TunnelBuildTimeMsPerHop + ms ) / 10.0 ) ), true );
        }
...
#if DEBUG
        private ConcurrentDictionary<string,int> NodeInactiveReason = new();
        private PeriodicAction ReportInactiveReason = new( TickSpan.Minutes( 7 ) );
        private HashSet<RouterStatistics> InactiveReasonAlreadyReported = new();
