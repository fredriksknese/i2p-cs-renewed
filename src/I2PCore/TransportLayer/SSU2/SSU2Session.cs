...
namespace I2PCore.TransportLayer.SSU2
{
    ...
    public class SSU2Session : ITransport
    {
        ...
        // Packet counters
        private uint SendPacketNumber = 0;
        private uint ReceivePacketNumber = 0;
        ...
        private int MaxPayloadSize = SSU2FragmentHandler.MAX_PAYLOAD_SIZE_IPV6;
        ...
        public SSU2Session(SSU2Host host, I2PRouterInfo remoteRouter, bool isOutgoing)
        {
            ...
            LocalConnectionId = ((ulong)BufUtils.RandomUint() | ((ulong)BufUtils.RandomUint() << 32));

            TransportConnectionLogger.Inst.Log($"Created outbound SSU2 session to {RemoteRouterInfo?.Identity?.IdentHash?.Id32Short}", RemoteRouterInfo?.Identity?.IdentHash?.Id32Short, "SSU2", "Outbound");
        }
        ...
        public void Connect()
        {
            ...
            try
            {
                ...
                TransportConnectionLogger.Inst.Log($"Initiating SSU2 connection to {RemoteEndpoint}", RemoteRouterInfo?.Identity?.IdentHash?.Id32Short, "SSU2", "Outbound");
        ...
        private void SendFragmented(byte[] i2npData, I2NpMessage msg)
        {
            ...
            Logging.LogDebug($"{DebugId}: Fragmenting I2NP message into {fragmentBlocks.Count} fragments ({i2npData.Length} bytes)");
            ...
            foreach (var blockData in fragmentBlocks)
            {
                ...
                var header = new Messages.SSU2Header
                {
                    IsLongHeader = false,
        ...
        public void Terminate(string reason = null)
        {
            ...
            TransportConnectionLogger.Inst.Log(logMsg, RemoteRouterInfo?.Identity?.IdentHash?.Id32Short, "SSU2", IsOutgoing ? "Outbound" : "Inbound");
        ...
        private void ExtractRemoteEndpoint()
        {
            ...
            var ssu2Address = RemoteRouterInfo?.Addresses?.FirstOrDefault(a =>
                ...
                && (RouterContext.UseIpV6 || !IPAddress.TryParse(a.Options["host"], out var ip) || ip.AddressFamily != System.Net.Sockets.AddressFamily.InterNetworkV6));

            if (ssu2Address == null)
                throw new ArgumentException("No suitable SSU2 address with host/port found in RouterInfo (IPv6 might be disabled)");
        ...
        private void SendSessionRequest()
        {
            ...
            while (packet == null)
            {
                ...
                if ((obfuscatedKey[0] & 0x80) == 0 && (ephKey[31] & 0x80) == 0)
                {
                    ...
                    Array.Copy(encryptedPayload, 0, packet, headerBytes.Length + obfuscatedKey.Length, encryptedPayload.Length);
        ...
        private byte[] BuildRequestPayload()
        {
            ...
            stream.Write(BufUtils.Flip16B((ushort)0));
        ...
        private void ProcessSessionConfirmed(byte[] packetData)
        {
            ...
            var chainingKey = NoiseState.GetChainingKey();
            var kHeader2 = Crypto.SSU2HeaderEncryption.DeriveSessionConfirmedHeaderKey(chainingKey);
            ...
            int totalFragments = header.Flags0 & 0x0F;
            int fragmentNumber = (header.Flags0 >> 4) & 0x0F;
            ...
            if ( totalFragments == 2 )
            {
                ...
                if ( fragmentNumber == 0 )
                {
                    ...
                    if ( _sessionConfirmedFragmentBuffer != null && _sessionConfirmedFragmentIsSecond )
                    {
                        ...
                        Array.Copy(_sessionConfirmedFragmentBuffer, 0, combined, payloadAfterHeader.Length, _sessionConfirmedFragmentBuffer.Length);
                ...
                else if ( fragmentNumber == 1 )
                {
                    ...
                    if ( _sessionConfirmedFragmentBuffer != null && !_sessionConfirmedFragmentIsSecond )
                    {
                        ...
                        Array.Copy(payloadAfterHeader, 0, combined, _sessionConfirmedFragmentBuffer.Length, payloadAfterHeader.Length);
            ...
            var confirmed = new Messages.SessionConfirmed();
            try
            {
            ...
            var sendKeyData = NoiseKDF.HKDF(sendKey, Array.Empty<byte>(),
                System.Text.Encoding.ASCII.GetBytes("HKDFSSU2DataKeys"), 64);
            ...
            TransportConnectionLogger.Inst.Log("Session established", RemoteRouterInfo?.Identity?.IdentHash?.Id32Short, "SSU2", IsOutgoing ? "Outbound" : "Inbound");
        ...
        private void SendSessionCreated()
        {
            ...
            while (packet == null)
            {
                ...
                if ((obfuscatedKey[0] & 0x80) == 0 && (ephKey[31] & 0x80) == 0)
                {
                    ...
                    if (IsPQSession && RemoteKemPublicKey != null)
                    {
                        byte[] kemCiphertext, kemSharedSecret;
                        switch (PQVersion)
                        {
                            case 3: // ML-KEM-768
                                (kemCiphertext, kemSharedSecret) = I2PCore.Crypto.MLKEM.MLKEM768.Encapsulate(RemoteKemPublicKey);
                                break;
                            default:
                                kemCiphertext = null;
        ...
        private void SendSessionConfirmed()
        {
            ...
            var maxPacketSize = RemoteEndpoint.AddressFamily == System.Net.Sockets.AddressFamily.InterNetworkV6
                ? SSU2FragmentHandler.SSU2_MAX_PACKET_SIZE - SSU2FragmentHandler.IPV6_HEADER_SIZE - SSU2FragmentHandler.UDP_HEADER_SIZE
                : SSU2FragmentHandler.SSU2_MAX_PACKET_SIZE - SSU2FragmentHandler.IPV4_HEADER_SIZE - SSU2FragmentHandler.UDP_HEADER_SIZE;
            ...
            bool needsFragmentation = encryptedPart2.Length > maxPayloadForPart2;
            ...
            if ( !needsFragmentation )
            {
                ...
                Crypto.SSU2HeaderEncryption.EncryptShortHeaderInPacket(packet, 0, kHeader1, kHeader2);
            ...
            else
            {
                ...
                Crypto.SSU2HeaderEncryption.EncryptShortHeaderInPacket(packet0, 0, kHeader1, kHeader2);
                ...
                Crypto.SSU2HeaderEncryption.EncryptShortHeaderInPacket(packet1, 0, kHeader1, kHeader2);
                ...
                Logging.LogDebug($"{DebugId}: SessionConfirmed fragmented: frag0={packet0.Length}B, frag1={packet1.Length}B");
            }
