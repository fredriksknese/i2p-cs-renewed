...
namespace I2PCore.TransportLayer.SSU2
{
    ...
    public class SSU2AckManager
    {
        ...
        /// </summary>
        private SortedSet<uint> ReceivedPackets = new SortedSet<uint>();

        /// <summary>
        /// Tracks packets sent but not yet acknowledged
        /// </summary>
        private Dictionary<uint, PendingPacket> UnackedPackets = new Dictionary<uint, PendingPacket>();

        /// <summary>
        /// Highest packet number we've sent ACK for
        /// </summary>
        private uint HighestAckedReceive = 0;
        ...
        public void ProcessAck(SSU2AckBlock ackBlock)
        {
            ...
            {
                for (uint pn = range.Start; pn <= range.End; pn++)
                {
            ...
            foreach (var pn in ackedPackets)
            {
                UnackedPackets.Remove(pn);
            }
        }
    ...
    {
        public List<AckRange> AckRanges = new List<AckRange>();
