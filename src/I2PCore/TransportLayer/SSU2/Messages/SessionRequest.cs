...
namespace I2PCore.TransportLayer.SSU2.Messages
{
    ...
    public class SessionRequest
    {
        ...
        public static SessionRequest Parse(BufRef data, byte[] bobIntroKey, byte[] fullPacket)
        {
            ...
            request.Header = SSU2Header.ParseLongHeader(new BufRef(fullPacket));
        ...
        public byte[] ToByteArray(byte[] bobIntroKey, byte[] ephemeralKey, byte[] encryptedPayload)
        {
            ...
            if (padding.Length > 0)
            {
                Array.Copy(padding, 0, packet, header.Length + obfuscatedX.Length + encryptedPayload.Length, padding.Length);
            }
