...
public static class SSU2Helpers
{
    ...
    public static bool ValidateConnectionIds(ulong sourceId, ulong destinationId)
    {
        ...
        {
            Logging.LogDebug(
                $"SSU2: Connection ID validation failed - source and destination are identical: {sourceId}");
            return false;
        }
        ...
        int mtu = 1280)
    {
        ...
        else
        {
            ...
            while (offset < routerInfoBytes.Length)
            {
                ...
                if (isFirstFragment) writer.WriteBytes(encryptedStaticKey);
    ...
    {
        var expectedNetId = (byte)I2PConstants.I2PNetworkId;
