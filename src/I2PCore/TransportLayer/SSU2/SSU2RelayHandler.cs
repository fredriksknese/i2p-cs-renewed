...
namespace I2PCore.TransportLayer.SSU2
{
    ...
    public class SSU2RelayHandler
    {
        ...
        public static readonly byte[] RELAY_REQUEST_PROLOGUE = System.Text.Encoding.ASCII.GetBytes("RelayRequestData");
        public static readonly byte[] RELAY_RESPONSE_PROLOGUE = System.Text.Encoding.ASCII.GetBytes("RelayAgreementOK");
        public static readonly byte[] PEER_TEST_PROLOGUE = System.Text.Encoding.ASCII.GetBytes("PeerTestValidate");
        ...
        private readonly ConcurrentDictionary<uint, RelaySession> PendingRelaySessions
            = new ConcurrentDictionary<uint, RelaySession>();

        // Manages active relay tags
        private readonly ConcurrentDictionary<uint, RelayTag> ActiveRelayTags
            = new ConcurrentDictionary<uint, RelayTag>();

        // Tracks pending peer test sessions by nonce
        private readonly ConcurrentDictionary<uint, PeerTestSession> PendingPeerTests
            = new ConcurrentDictionary<uint, PeerTestSession>();
        ...
        public void HandleRelayRequest(SSU2Session fromAlice, byte[] blockData)
        {
            ...
            var bobHash = SessionLayer.RouterContext.Inst.MyRouterIdentity.IdentHash;
            var charlieHash = tag.Session.RemoteRouterIdentity.IdentHash;
            
            if ( fromAlice.RemoteRouterIdentity?.SigningPublicKey != null )
            {
                if ( !Verify( RELAY_REQUEST_PROLOGUE, bobHash, charlieHash, request.RawSignedData, request.Signature, fromAlice.RemoteRouterIdentity.SigningPublicKey ) )
                {
                    Logging.LogWarning( $"SSU2Relay: RelayRequest signature verification failed from Alice {fromAlice.DebugId}" );
            ...
            {
                Logging.LogWarning( $"SSU2Relay: No signing public key for Alice {fromAlice.DebugId}, cannot verify RelayRequest" );
            }
        ...
        public void HandleRelayResponse(SSU2Session fromSession, byte[] blockData)
        {
            ...
            if (PendingRelaySessions.TryRemove(response.Nonce, out var relaySession))
            {
                ...
                {
                    if ( !Verify( RELAY_RESPONSE_PROLOGUE, bobHash, null, response.RawSignedData, response.Signature, relaySession.CharlieSession.RemoteRouterIdentity.SigningPublicKey ) )
                    {
                        Logging.LogWarning( $"SSU2Relay: RelayResponse signature verification failed from Charlie {relaySession.CharlieSession.DebugId}" );
                        // Protocol says to silently drop or reject if sig fails
                        return;
                    }
                }
        ...
        public void HandleRelayIntro(SSU2Session fromBob, byte[] blockData)
        {
            ...
            var mySigningKey = SessionLayer.RouterContext.Inst.PrivateSigningKey;

            // Build data part first to sign it
            var address = request?.Address ?? Array.Empty<byte>();
            var responsePayload = BuildRelayResponseData(0, RelayAccept, nonce, timestamp, SSU2Constants.VERSION, address, Array.Empty<byte>(), 0);
            ...
            if (address.Length > 0)
            {
                ...
                if (aliceEndpoint != null)
                {
                    SendHolePunch(aliceEndpoint, nonce, intro.AliceHash);
                }
            }
        }
        ...
        public void HandlePeerTest(SSU2Session fromSession, byte[] blockData)
        {
            ...
            var myHash = SessionLayer.RouterContext.Inst.MyRouterIdentity.IdentHash;
            ...
            switch (pt.MsgNum)
            {
                case 1:
                    ...
                    {
                        if ( !Verify( PEER_TEST_PROLOGUE, myHash, null, pt.RawSignedData, pt.Signature, fromSession.RemoteRouterIdentity.SigningPublicKey ) )
                        {
                            Logging.LogWarning( $"SSU2Relay: PeerTest msg 1 signature verification failed from Alice {fromSession.DebugId}" );
                            return;
                        }
                    }
                ...
                case 3:
                    ...
                    if ( fromSession?.RemoteRouterIdentity?.SigningPublicKey != null )
                    {
                        if ( PendingPeerTests.TryGetValue( pt.Nonce, out var ptSession ) )
                        {
                            ...
                            if ( !Verify( PEER_TEST_PROLOGUE, myHash, aliceHash, pt.RawSignedData, pt.Signature, fromSession.RemoteRouterIdentity.SigningPublicKey ) )
                            {
                                Logging.LogWarning( $"SSU2Relay: PeerTest msg 3 signature verification failed from Charlie {fromSession.DebugId}" );
                                return;
                            }
                        }
                    }
                ...
                case 5:
                    ...
                    {
                        if ( !Verify( PEER_TEST_PROLOGUE, myHash, null, pt.RawSignedData, pt.Signature, charlieRI5.Identity.SigningPublicKey ) )
                        {
                            Logging.LogWarning( $"SSU2Relay: PeerTest msg 5 signature verification failed from Charlie {charlieHash5.Id32Short}" );
                            return;
                        }
                    }
                ...
                case 6:
                    ...
                    {
                        if ( !Verify( PEER_TEST_PROLOGUE, myHash, null, pt.RawSignedData, pt.Signature, aliceRI6.Identity.SigningPublicKey ) )
                        {
                            Logging.LogWarning( $"SSU2Relay: PeerTest msg 6 signature verification failed from Alice {aliceHash6.Id32Short}" );
                            return;
                        }
                    }
                ...
                case 7:
                    ...
                    if ( fromSession?.RemoteRouterIdentity?.SigningPublicKey != null )
                    {
                        if ( PendingPeerTests.TryGetValue( pt.Nonce, out var ptSession ) )
                        {
                            ...
                            if ( !Verify( PEER_TEST_PROLOGUE, myHash, aliceHash, pt.RawSignedData, pt.Signature, fromSession.RemoteRouterIdentity.SigningPublicKey ) )
                            {
                                Logging.LogWarning( $"SSU2Relay: PeerTest msg 7 signature verification failed from Charlie {fromSession.DebugId}" );
                                return;
                            }
                        }
                    }
        ...
        private void HandlePeerTestMsg1(SSU2Session fromAlice, PeerTestData pt)
        {
            ...
            var signature = Sign( PEER_TEST_PROLOGUE, charlieHash, aliceHash, signedPart, SessionLayer.RouterContext.Inst.PrivateSigningKey );
            ...
            Logging.LogDebug( $"SSU2Relay: PeerTest msg1 from Alice, forwarded as msg2 to Charlie {charlie.DebugId}, nonce={pt.Nonce}" );
        }

        private void SendPeerTestResponse( SSU2Session session, uint nonce, byte code, byte[] hash, byte flags, byte[] address )
        {
            var timestamp = (uint)DateTimeOffset.UtcNow.ToUnixTimeSeconds();
            var data = BuildPeerTestData( 2, code, flags, hash, SSU2Constants.VERSION, nonce, timestamp, address, Array.Empty<byte>() );
        ...
        private void HandlePeerTestMsg2(SSU2Session fromBob, PeerTestData pt)
        {
            ...
            var signedPart3 = BuildPeerTestData(
                ...
                Array.Empty<byte>(),
                null );
            var signature3 = Sign( PEER_TEST_PROLOGUE, bobHash, aliceHash, signedPart3, SessionLayer.RouterContext.Inst.PrivateSigningKey );
            ...
            var signedPart5 = BuildPeerTestData(
                5, PeerTestAccept, 0,
                SessionLayer.RouterContext.Inst.MyRouterIdentity.IdentHash.Hash.ToByteArray(),
                SSU2Constants.VERSION,
                pt.Nonce,
                timestamp,
                pt.Address, 
                null );
            var signature5 = Sign( PEER_TEST_PROLOGUE, aliceHash, null, signedPart5, SessionLayer.RouterContext.Inst.PrivateSigningKey );
        ...
        private void HandlePeerTestMsg3(SSU2Session fromCharlie, PeerTestData pt)
        {
            ...
            var signedPart4 = BuildPeerTestData(
                ...
                pt.Address,
                null );
            var signature4 = Sign( PEER_TEST_PROLOGUE, aliceHash, charlieHash, signedPart4, SessionLayer.RouterContext.Inst.PrivateSigningKey );
        ...
        private void HandlePeerTestMsg5(PeerTestData pt)
        {
            ...
            var signedPart6 = BuildPeerTestData(
                ...
                pt.Address,
                null );
            var signature6 = Sign( PEER_TEST_PROLOGUE, charlieHash, null, signedPart6, SessionLayer.RouterContext.Inst.PrivateSigningKey );
        ...
        private void HandlePeerTestMsg6(PeerTestData pt)
        {
            ...
            var signedPart7 = BuildPeerTestData(
                ...
                pt.Address,
                null );
            var signature7 = Sign( PEER_TEST_PROLOGUE, bobHash, aliceHash, signedPart7, SessionLayer.RouterContext.Inst.PrivateSigningKey );
        ...
        private void SendHolePunch(IPEndPoint aliceEndpoint, uint nonce, byte[] aliceHash)
        {
            ...
            ulong connId = (ulong)nonce | ((ulong)nonce << 32);
            ...
            // Send with retries
            for (int attempt = 0; attempt < HolePunchMaxRetries; attempt++)
            {
                ...
                if (attempt < HolePunchMaxRetries - 1)
                {
                    System.Threading.Thread.Sleep(HolePunchRetryIntervalMs);
                }
            }
        }
        ...
        public bool InitiateRelayRequest( SSU2Session bobSession, I2PIdentHash charlieHash, uint relayTag )
        {
            ...
            Logging.LogInformation( $"SSU2Relay: Initiated RelayRequest for Charlie {charlieHash.Id32Short} via Bob {bobSession.DebugId}" );
            return true;
        }
        ...
        internal void ProcessPeerTestResult( uint nonce, byte code, byte[] address )
        {
            ...
            {
                session.ReportedAddress = ParseEndpointFromAddress( address );
            }

            bool isReachable = code == PeerTestAccept;
            ...
            SessionLayer.RouterContext.Inst.IsFirewalled = !isReachable;
        ...
        private static IPEndPoint ParseEndpointFromAddress(byte[] addr)
        {
            ...
            {
                // IPv4
                var ip = new IPAddress(new byte[] { addr[0], addr[1], addr[2], addr[3] });
