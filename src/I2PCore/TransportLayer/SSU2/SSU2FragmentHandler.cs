...
namespace I2PCore.TransportLayer.SSU2
{
    ...
    public class SSU2FragmentHandler
    {
        ...
        public const int MAX_PAYLOAD_SIZE_IPV6 = SSU2_MAX_PACKET_SIZE - IPV6_HEADER_SIZE - UDP_HEADER_SIZE - CRYPTO_OVERHEAD;
        public const int MAX_PAYLOAD_SIZE_IPV4 = SSU2_MAX_PACKET_SIZE - IPV4_HEADER_SIZE - UDP_HEADER_SIZE - CRYPTO_OVERHEAD;
        ...
        private class IncompleteMessage
        {
            ...
            public SortedList<int, Fragment> OutOfOrderFragments = new();
        }
        ...
        public static List<byte[]> FragmentMessage(byte[] i2npData, uint msgId, int maxPayloadSize)
        {
            ...
            int firstFragDataSize = maxPayloadSize - FIRST_FRAGMENT_HEADER;
            ...
            while (offset < i2npData.Length)
            {
                ...
                int remaining = i2npData.Length - offset;
                int followOnDataSize = maxPayloadSize - FOLLOWON_FRAGMENT_OVERHEAD;
                bool isLast = remaining <= followOnDataSize;
                if (!isLast)
                {
                ...
                int blockDataSize = followOnDataSize + 5;
        ...
        public byte[] HandleFirstFragment(byte[] data)
        {
            ...
            uint msgId = (uint)(data[1] << 24 | data[2] << 16 | data[3] << 8 | data[4]);
        ...
        public byte[] HandleFollowOnFragment(byte[] data)
        {
            ...
            byte flags = data[0];
            int fragmentNum = flags >> 1;
            bool isLast = (flags & 1) == 1;
            uint msgId = (uint)(data[1] << 24 | data[2] << 16 | data[3] << 8 | data[4]);
        ...
        public void CleanUp()
        {
            ...
            foreach (var kvp in _incompleteMessages)
            {
                if (now - kvp.Value.LastFragmentTime > INCOMPLETE_MESSAGE_TIMEOUT_SECONDS)
                {
                    ...
                    {
                        Logging.LogWarning($"SSU2 Fragment: incomplete message {kvp.Key:X8} timed out after {INCOMPLETE_MESSAGE_TIMEOUT_SECONDS}s");
                    }
                }
            }
        }
