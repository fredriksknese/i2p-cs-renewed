...
public class NTCP2AckManager
{
    ...
    /// </summary>
    private readonly Dictionary<uint, PendingMessage> PendingMessages = new();

    /// <summary>
    ///     Tracks received message IDs to detect duplicates
    /// </summary>
    private readonly HashSet<uint> ReceivedMessageIds = new();
    ...
    public void RecordReceived(uint messageId)
    {
        ...
        if (ReceivedMessageIds.Count > 10000)
        {
            ...
            foreach (var id in toRemove) ReceivedMessageIds.Remove(id);
        }
    }
    ...
    private void CleanupOldPending()
    {
        ...
        foreach (var id in toRemove) PendingMessages.Remove(id);
    }
