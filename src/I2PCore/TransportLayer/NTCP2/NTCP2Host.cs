...
public class NTCP2Host : ITransportProtocol
{
    ...
    private readonly List<NTCP2Session> Sessions = new();
    ...
    public ITransport AddSession(I2PRouterInfo router)
    {
        ...
        lock (SessionsLock)
        {
            outboundCount = Sessions.Count(s => s.IsOutgoing);
        }
        ...
        {
            Logging.LogWarning(
                $"NTCP2Host: TERMINATION REASON [C#-BOB]: Outbound connection limit reached ({maxOutbound}), rejecting outbound to {router.Identity?.IdentHash?.Id32Short}");
            return null;
        }
        ...
        lock (SessionsLock)
        {
            ...
            foreach (var session in allSessions)
                try
                {
                    ...
                    if (session.IsTerminated) Sessions.Remove(session);
                }
                catch (Exception ex)
                {
    ...
    private void InitializeStaticKeys()
    {
        ...
        if (loadedKeys.HasValue)
        {
            ...
            Logging.LogInformation(
                $"NTCP2Host: Loaded static keys. pub[0:4]={BitConverter.ToString(StaticPublicKey, 0, 4).Replace("-", "")} derivedPub[0:4]={BitConverter.ToString(derivedPub, 0, 4).Replace("-", "")} keyPairMatch={match}");
        }
        else
        {
            ...
            Logging.LogInformation(
                $"NTCP2Host: Generated valid static key (MSB clear) after {attempts} attempt(s). pub[0:4]={BitConverter.ToString(StaticPublicKey, 0, 4).Replace("-", "")} keyPairMatch={match2}");
            ...
            while (!MyCancellationToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    while (!MyCancellationToken.IsCancellationRequested && !_listenerRestartRequested)
                    {
                        ...
                        {
                            Logging.LogWarning(
                                $"NTCP2Host: TERMINATION REASON [C#-BOB]: IP BLOCKED - rejecting connection from {remoteEP} (DecayingIpBlockFilter triggered)");
                            client.Close();
                            continue;
                        }
                        ...
                        lock (SessionsLock)
                        {
                            inboundCount = Sessions.Count(s => !s.IsOutgoing);
                        }
                        ...
                        {
                            Logging.LogWarning(
                                $"NTCP2Host: TERMINATION REASON [C#-BOB]: Inbound connection limit reached ({maxInbound}), rejecting {remoteEP}");
                            client.Close();
                            continue;
                        }
                        ...
                        Logging.LogInformation(
                            $"NTCP2Host: [C#-BOB] Accepted incoming connection from {remoteEP}, session={session.DebugId}");
                ...
                if (!MyCancellationToken.IsCancellationRequested)
                    Logging.LogWarning(
                        "NTCP2Host: [C#-BOB] Accept loop exited - RESTARTING listener. This gap means i2pd connections during restart will get EOF.");
            }
        }
        finally
        {
            ...
            stream.BeginRead(buffer, 0, buffer.Length, ar =>
            {
                try
                {
                    ...
                    if (bytesRead > 0)
                    {
                        ...
                        if (!session.IsTerminated) StartAsyncReceive(session);
                    }
                    else
                    {
                        // Connection closed
    ...
    /// </summary>
    public void ReportProblem(IPAddress addr)
    {
    ...
    private void UpdateRouterContext()
    {
        ...
        if (publishedAddress == null || publishedAddress.Equals(IPAddress.Any) ||
            publishedAddress.Equals(IPAddress.IPv6Any))
        {
