...
public class NTCP2SecurityValidator
{
    ...
    private static readonly ConcurrentDictionary<string, (int count, DateTime expiry)> ipBlacklist = new();
    ...
    public static bool ValidateTimestamp(uint timestamp, int maxDelta = MAX_TIME_DELTA)
    {
        ...
        if (!isValid)
            Logging.LogWarning(
                $"NTCP2SecurityValidator: Timestamp validation failed - delta={delta:F1}s (max={maxDelta}s), message={messageTime:yyyy-MM-dd HH:mm:ss} UTC, current={currentTime:yyyy-MM-dd HH:mm:ss} UTC");
    ...
    private static void CleanupReplayCache()
    {
        ...
        foreach (var key in keysToRemove) replayCache.TryRemove(key, out _);
    }
    ...
    public static void RecordFailure(string ipAddress)
    {
        ...
        ipBlacklist.AddOrUpdate(
            ...
            (key, existing) =>
            {
                ...
                if (existing.expiry > now) return (count: existing.count + 1, existing.expiry);
    ...
    public static bool IsBlacklisted(string ipAddress)
    {
        if (ipBlacklist.TryGetValue(ipAddress, out var entry))
        {
            ...
            if (entry.expiry > DateTime.UtcNow) return entry.count >= MAX_FAILURES_BEFORE_BAN;
    ...
    /// </summary>
    public static bool ValidateStaticKeyMatchesRouterInfo(byte[] staticKey, I2PRouterInfo routerInfo)
    {
