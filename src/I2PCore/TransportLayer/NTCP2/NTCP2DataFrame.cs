...
public class NTCP2DataFrame
{
    ...
    public static NTCP2DataFrame BuildWithRouterInfo(I2PRouterInfo ri, bool includeDateTime = true)
    {
        ...
        if (includeDateTime) frame.AddBlock(new NTCP2DateTimeBlock());
    ...
    public byte[] ToByteArray()
    {
        ...
        foreach (var block in Blocks)
        {
            if (3 + block.Data.Length > writer.Remaining)
            {
                ...
                Logging.LogWarning(
                    $"NTCP2DataFrame: Block of size {block.Data.Length} cannot fit in remaining {writer.Remaining} bytes.");
                break;
            }
    ...
    public byte[] BuildEncryptedFrame(NoiseXK noiseState, NTCP2SipHash sipHash)
    {
        ...
        var frameLength = (ushort)encrypted.Length;
        Length = frameLength;
        ...
        Logging.LogDebug(
            $"NTCP2DataFrame: plaintext={payload.Length}B, encrypted={encrypted.Length}B, frameLen={frameLength}, obfLen=0x{obfuscatedLength:X4}");
    ...
    private static void ParseAndValidateBlocks(byte[] decrypted, NTCP2DataFrame frame)
    {
        ...
        {
            var remaining = blockReader.Remaining;
            if (remaining < 3)
