    ...
    public static class NTCP2ProbingResistance
    {
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                {
                    NTCP2SecurityValidator.RecordFailure(ipAddress);
                }

                // Generate random delay
                int delayMs = random.Next(MIN_DELAY_MS, MAX_DELAY_MS);
                ...
                {
                    int bytesToRead = random.Next(MIN_BYTES_TO_READ, MAX_BYTES_TO_READ);
                    byte[] buffer = new byte[Math.Min(bytesToRead, 8192)];
