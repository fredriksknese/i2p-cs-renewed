    ...
    public class NoiseKDF
    {
        ...
        public void InitializeSymmetric(string protocolName)
        {
            if (protocolName.Length <= HashLength)
            {
                ...
                var nameBytes = System.Text.Encoding.ASCII.GetBytes(protocolName);
            ...
            else
            {
                ...
                {
                    Hash = sha256.ComputeHash(System.Text.Encoding.ASCII.GetBytes(protocolName));
                }
            }
            ...
            // Debug logging
            I2PCore.Utils.Logging.LogDebugData($"NoiseKDF InitializeSymmetric: protocolName length={protocolName.Length}");
            I2PCore.Utils.Logging.LogDebugData($"NoiseKDF: Initial Hash = {BitConverter.ToString(Hash).Replace("-", "")}");
            I2PCore.Utils.Logging.LogDebugData($"NoiseKDF: Initial CK   = {BitConverter.ToString(ChainingKey).Replace("-", "")}");
        }
        ...
        public void MixHash(byte[] data)
        {
            using (var sha256 = SHA256.Create())
            {
                ...
                {
                    Array.Copy(data, 0, input, Hash.Length, data.Length);
                }
                Hash = sha256.ComputeHash(input);
            }
            I2PCore.Utils.Logging.LogDebugData($"NoiseKDF MixHash: dataLen={data?.Length ?? 0}, newHash = {BitConverter.ToString(Hash).Replace("-", "").Substring(0, 32)}...");
        }
        ...
        public void SetHash(byte[] newHash)
        {
            ...
            I2PCore.Utils.Logging.LogDebugData($"NoiseKDF SetHash: newHash = {BitConverter.ToString(newHash).Replace("-", "").Substring(0, 32)}...");
        }
