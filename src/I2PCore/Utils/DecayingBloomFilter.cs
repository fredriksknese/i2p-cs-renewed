...
namespace I2PCore.Utils
{
    ...
    public class DecayingBloomFilter : IDisposable
    {
        ...
        private int GetBit(byte[] data, int hashIndex)
        {
            ...
            uint hash = MurmurHash3.Hash32(data, (uint)hashIndex);
    ...
    public static class MurmurHash3
    {
        public static uint Hash32(byte[] data, uint seed = 0)
        {
            ...
            {
                uint k = (uint)(data[i * 4] | data[i * 4 + 1] << 8 | data[i * 4 + 2] << 16 | data[i * 4 + 3] << 24);
                k *= c1;
                k = RotateLeft(k, 15);
                k *= c2;
                h ^= k;
            ...
            switch (length & 3)
            {
                case 3: tail ^= (uint)data[tailOffset + 2] << 16; goto case 2;
                case 2: tail ^= (uint)data[tailOffset + 1] << 8; goto case 1;
                case 1:
