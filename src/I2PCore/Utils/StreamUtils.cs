...
public static class StreamUtils
{
    ...
    /// <summary>
    ///     Override for the application data path. When set, this is used
    ///     instead of the default (~/.config/I2Pz). Used for test isolation.
    /// </summary>
    public static string AppPathOverride { get; set; }

    public static string AppPath
    {
        get
        {
            if (!string.IsNullOrEmpty(AppPathOverride))
    ...
    public static long CopyStream(Stream dest, Stream src, CopyProgressCallback cb, long maxlen, int bufsize)
    {
        ...
        while ((read = src.Read(buffer, 0,
                   maxlen == -1 ? buffer.Length : (int)Math.Min(buffer.Length, maxlen - bytescopied))) > 0)
        {
            dest.Write(buffer, 0, read);
            bytescopied += read;
            if (cb != null) cb(bytescopied);
        }
    ...
    public static int ReadInt32(Stream src)
    {
        unchecked
        {
            return ReadUInt16(src) | (ReadUInt16(src) << 16);
        }
    }
