...
public class TimeWindowDictionary<T, TV> : IDisposable, IEnumerable<KeyValuePair<T, TV>> where TV : class
{
    ...
    private readonly ConcurrentDictionary<T, KeyValuePair<TV, TickCounter>> Memory = new();
    ...
    {
        if (LastCleanup.DeltaToNowSeconds > SecondsBetweenCleanup) Cleanup();
    }
    ...
    public void Touch(T ident)
    {
        ...
        if (Memory.TryGetValue(ident, out var pair)) pair.Value.SetNow();
    }
    ...
    protected bool RemoveAndDispose(T ident)
    {
        ...
        if (result)
        {
            if (removed.Key is IDisposable) ((IDisposable)removed.Key).Dispose();
            if (removed.Value is IDisposable) ((IDisposable)removed.Value).Dispose();
        }
    ...
    public bool TryRemove(T ident, out TV value)
    {
        ...
        value = result ? removed.Key : default;
        return result;
    }
    ...
    {
        if (Memory.TryGetValue(key, out var pair)) action(key, pair.Key);
    }
    ...
    {
        foreach (var identpair in Memory.ToArray()) RemoveAndDispose(identpair.Key);
    }
