...
public class ECIESGarlicProcessor
{
    ...
    private DeliveryInstructions ParseDeliveryInstructions(I2PBufferCursor reader)
    {
        ...
        // Read delay (optional)
        if ((flags & 0x04) != 0) instructions.Delay = reader.ReadUInt32BigEndian();
    ...
    private CloveResult ProcessClove(GarlicClove clove)
    {
        try
        {
            ...
            if (result != null) result.Expiration = clove.Expiration;
            return result;
        }
        catch (Exception ex)
        {
    ...
    public static byte[] BuildGarlicMessage(List<GarlicClove> cloves)
    {
        ...
        // Write each clove
        foreach (var clove in cloves) WriteClove(stream, clove);
    ...
    {
        // Build flags
        var flags = (byte)instructions.DeliveryType;
        ...
        if (instructions.Delay.HasValue) stream.WriteBlock(BufUtils.Flip32Bl(instructions.Delay.Value));
    }
}
