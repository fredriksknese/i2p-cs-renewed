...
public class ECIESRouterSKM
{
    ...
    private readonly ConcurrentDictionary<SessionTag, TagInfo> _oneTimeSessions = new();
    ...
    public byte[] CreateNewSessionMessage(I2PIdentHash routerHash, byte[] routerPublicKey, byte[] payload)
    {
        ...
        var currentSize = 10 + payload.Length;
    ...
    private byte[] BuildMessageWithTags(List<SessionTag> tags, byte[] payload)
    {
        ...
        // Write tags
        foreach (var tag in tags) stream.WriteBytes(tag.ToByteArray());
    ...
    private class RouterSession
    {
        ...
        public ConcurrentDictionary<SessionTag, TagInfo> InboundTags { get; }
