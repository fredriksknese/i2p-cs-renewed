...
public class EgaesSessionKeyOrigin
{
    ...
    private readonly TimeWindowDictionary<I2PSessionTag, I2PSessionKey> AckedTags = new(AcKedTagLifetime);

    private readonly ClientDestination Context;
    ...
    internal void DeliveryStatusReceived(DeliveryStatusMessage msg, InboundTunnel from)
    {
        if (NotAckedTags.TryRemove(msg.StatusMessageId, out var tags))
        {
            ...
            foreach (var tag in tags.Tags) AckedTags[tag.Key] = tags.SessionKey;
        }
    }
        ...
        IList<GarlicClove> cloves)
    {
        ...
        if (pkey == null)
            throw new InvalidOperationException(
                $"{this}: No ElGamal public key available for {RemoteDestination.Id32Short}");
    ...
    private ( I2PSessionTag, I2PSessionKey ) PopAckedTag()
    {
    ...
    private SessionAndTags GenerateNewTags(IList<GarlicClove> cloves, InboundTunnel replytunnel)
    {
        ...
        for (var i = 0; i < NewTagsWhenGenerating; ++i) sat.Tags[new I2PSessionTag()] = 1;
    ...
    private class SessionAndTags
    {
        public readonly TimeWindowDictionary<I2PSessionTag, object> Tags = new(SentTagLifetime);
