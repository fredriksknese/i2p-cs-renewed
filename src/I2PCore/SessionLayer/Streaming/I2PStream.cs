...
namespace I2PCore.SessionLayer.Streaming
{
    ...
    public class I2PStream : IDisposable
    {
        ...
        public uint RecvStreamId { get; private set; }
        ...
        {
            Logging.LogDebug($"I2PStream {RecvStreamId:X8}: HandleNextPacket: Seq {packet.SequenceNumber} Ack {packet.AckThrough} Flags {packet.Flags}");
            ...
            {
                Logging.LogWarning($"I2PStream {RecvStreamId:X8}: ReceiveStreamId mismatch! Got {packet.ReceiveStreamId:X8}, expected {RecvStreamId:X8}. Dropping packet.");
                return;
            }
        ...
        private void ParseSynOptions(StreamingPacket packet)
        {
            ...
            if ((flags & StreamingPacket.FLAG_SIGNATURE_INCLUDED) != 0 && _remoteDestination != null)
            {
                try
                {
                    ...
                    int sigLen = verifyKey.Certificate.SignatureLength;
        ...
        private void ProcessAck(StreamingPacket packet)
        {
            ...
            if (anyAcked)
            {
                ...
                if ( LOSS_BASED_CONTROL_ENABLED )
                {
                    ...
                    {
                        double lossRate = (double)_lossCount / _totalSentForLoss;
            ...
            if ((packet.Flags & StreamingPacket.FLAG_DELAY_REQUESTED) != 0 && packet.OptionData != null)
            {
                ...
                {
                    ushort delay = (ushort)((packet.OptionData[0] << 8) | packet.OptionData[1]);
                    DetectChoking(delay);
                }
            }
        ...
        private void WindowIncrease(int numAcked)
        {
            ...
            int ssThresh = _windowDropTargetSize > 0 ? _windowDropTargetSize : MAX_WINDOW_SIZE;
        ...
        private void ResetWindowSize()
        {
            ...
            if (_lastWindowDropSize > 0)
            {
                ...
                double recoveryTime = _rtt * _lastWindowDropSize * PREV_SPEED_KEEP_TIME_COEFF;
        ...
        private void UpdateRTT(int rttSample)
        {
            ...
            double dev = Math.Abs(rttSample - _rtt);
            _jitterAccum += dev;
            _jitterDiv++;
            if (_jitterDiv >= 4)
            {
        ...
        {
            int available = _windowSize - _sentPackets.Count;
            if (available <= 0)
            {
                Logging.LogDebug($"I2PStream {RecvStreamId:X8}: Window full (size {_windowSize}, sent {_sentPackets.Count})");
                return;
            }

            while (available > 0 && _sendBuffer.TryDequeue(out var data))
            {
                Logging.LogDebug($"I2PStream {RecvStreamId:X8}: Dequeued {data.Length} bytes for sending. available {available}");
                int offset = 0;
                int mtu = _negotiatedMtu;
                while (offset < data.Length && available > 0)
                {
                    int chunkSize = Math.Min(data.Length - offset, mtu);
                    ...
                    Logging.LogDebug($"I2PStream {RecvStreamId:X8}: Built data packet seq {pkt.SequenceNumber} payload {pkt.Payload.Length} bytes");
                    SendPacket(pkt);
                    available--;
                    offset += chunkSize;
                }
        ...
        private StreamingPacket BuildDataPacket(byte[] payload)
        {
            ...
            if (pkt.SequenceNumber == 0 && Status == StreamStatus.New)
            {
                ...
                if (_signingPrivateKey != null)
                {
                    ...
                    int sigLen = _signingPrivateKey.Certificate.SignatureLength;
        ...
        private void SendPacket(StreamingPacket pkt)
        {
            ...
            Logging.LogDebug($"I2PStream {RecvStreamId:X8}: Calling sendCallback for pkt seq {pkt.SequenceNumber} ({bytes.Length} bytes). SendStreamId: {pkt.SendStreamId:X8}, ReceiveStreamId: {pkt.ReceiveStreamId:X8}, AckThrough: {pkt.AckThrough}, Flags: {pkt.Flags:X4}");
            _sendCallback(bytes);
        ...
        private void SignPacket(StreamingPacket pkt)
        {
            ...
            int sigOffset = pkt.OptionData.Length - sigLen;
            for (int i = 0; i < sigLen; i++) pkt.OptionData[sigOffset + i] = 0;
        ...
        private void ScheduleAck()
        {
            ...
            int delay = Math.Max(MIN_SEND_ACK_TIMEOUT, (int)(_rtt / 10));
            _ackTimer?.Dispose();
        ...
        private void HandleResendTimer()
        {
            ...
            if (toResend.Count == 0 && _sentPackets.Count > 0)
            {
                ...
                {
                    Logging.LogDebug($"I2PStream {RecvStreamId:X8}: Timeout retransmit seq {oldest.SequenceNumber}. attempts {_numResendAttempts}");
                    _numResendAttempts++;
                    ...
                    if (oldest.HasSignature && _signingPrivateKey != null)
                    {
                        // Signature covers entire packet with signature field zeroed
                        int sigLen = _signingPrivateKey.Certificate.SignatureLength;
                        int sigOffset = oldest.OptionData.Length - sigLen;
                        var sigBytes = new byte[sigLen];
