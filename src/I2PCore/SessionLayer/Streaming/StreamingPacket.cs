...
namespace I2PCore.SessionLayer.Streaming
{
    ...
    public class StreamingPacket
    {
        ...
        public static StreamingPacket Parse(byte[] data)
        {
            ...
            pkt.SendStreamId = ReadBE32(data, offset); offset += 4;
            pkt.ReceiveStreamId = ReadBE32(data, offset); offset += 4;
            pkt.SequenceNumber = ReadBE32(data, offset); offset += 4;
            pkt.AckThrough = ReadBE32(data, offset); offset += 4;

            byte nackCount = data[offset++];
            ...
            pkt.Flags = ReadBE16(data, offset); offset += 2;

            ushort optionSize = ReadBE16(data, offset); offset += 2;
        ...
        {
            int nackBytes = NACKs.Count * 4;
            int headerSize = 4 + 4 + 4 + 4 + 1 + nackBytes + 1 + 2 + 2 + OptionData.Length;
            int totalSize = headerSize + Payload.Length;
            var result = new byte[totalSize];
            int offset = 0;

            WriteBE32(result, offset, SendStreamId); offset += 4;
            WriteBE32(result, offset, ReceiveStreamId); offset += 4;
            WriteBE32(result, offset, SequenceNumber); offset += 4;
            WriteBE32(result, offset, AckThrough); offset += 4;
            ...
            WriteBE16(result, offset, Flags); offset += 2;
            WriteBE16(result, offset, (ushort)OptionData.Length); offset += 2;
