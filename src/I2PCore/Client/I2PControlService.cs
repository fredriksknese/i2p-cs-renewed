...
public class I2PControlService : IDisposable
{
    ...
    private string ProcessJsonRpcRequest(string requestJson)
    {
        try
        {
            ...
            if (!ValidateToken(parameters)) return BuildErrorResponse(id, -32003, "Invalid or expired token");
                        ...
                        ThreadPool.QueueUserWorkItem(_ =>
                        {
                            try
                            {
                                ...
                                Logging.LogInformation(
                                    "I2PControlService: Graceful shutdown - waiting 60s for tunnels to expire");
                                Thread.Sleep(60000);
                                Router.Stop();
                                ClientContext.Inst?.Stop();
                            }
    ...
    {
        return
            $"{{\"id\":{id},\"error\":{{\"code\":{code},\"message\":\"{EscapeJsonString(message)}\"}},\"jsonrpc\":\"2.0\"}}";
    }
