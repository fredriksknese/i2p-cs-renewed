...
public class HTTPProxy : IDisposable
{
    ...
    {
        ClientDestination = clientDestination ?? throw new ArgumentNullException(nameof(clientDestination));
        _streamingDestination = streamingDestination ?? throw new ArgumentNullException(nameof(streamingDestination));
        _listenPort = listenPort;
    }

    public ClientDestination ClientDestination { get; }
            ...
            using (tcpClient)
            {
                        ...
                        ClientDestination.SignedLeases == null) && tunnelWaitAttempts < 60)
                {
                    if (tunnelWaitAttempts == 0)
                        HttpProxyLogger.Inst.Log(method, target, "Connecting",
                            "Waiting for client tunnels to be established...");
                     ...
                     ClientDestination.SignedLeases == null))
                {
                    var reason = ClientDestination.SignedLeases == null
                        ? "LeaseSet not ready"
                        : "No established tunnels";
                    Logging.LogWarning($"HTTPProxy: Timeout waiting for tunnels for {hostname} ({reason})");
                    await SendError(stream, 504,
                        $"I2P tunnels not ready ({reason}). Please try again in a few moments.");
                    return;
                }
                ...
                try
                {
                    ...
                    HttpProxyLogger.Inst.Log(method, target, "Connecting",
                        $"Created I2P stream {i2pStream.RecvStreamId:X8} to {hostname}");
                }
                catch (Exception ex)
                {
    ...
    private async Task<I2PDestination> ResolveB32Async(string hostname, CancellationToken ct)
    {
        try
        {
            ...
            ClientDestination.LookupDestination(
                identHash,
                (hash, leaseSet, tag) =>
                {
                    if (leaseSet != null)
                    {
                        Logging.LogInformation(
                            $"HTTPProxy: b32 resolved {hostname} → LeaseSet with {leaseSet.Leases.Count()} leases");
    ...
    private static async Task<byte[]> ReadHttpHeader(NetworkStream stream, CancellationToken ct)
    {
        ...
        while (totalRead < HeaderMaxBytes)
        {
            ...
            for (var i = Math.Max(0, totalRead - bytesRead - 3); i <= totalRead - 4; i++)
        ...
        string target, byte[] initialData, CancellationToken ct)
    {
        ...
        // I2P -> TCP worker
        var i2pDataQueue = Channel.CreateUnbounded<byte[]>();
            ...
            try
            {
                await foreach (var data in i2pDataQueue.Reader.ReadAllAsync(linkedCts.Token))
                {
                    if (!isFirstDataReceived)
                    {
                        ...
                        HttpProxyLogger.Inst.Log(method, target, "Streaming",
                            $"First data packet received from I2P: {data.Length} bytes.");
                    }

                    // Capture first 10KB for logging
                    if (responseData.Length < 10240)
                        responseData.Append(Encoding.UTF8.GetString(data, 0,
                            Math.Min(data.Length, 10240 - responseData.Length)));
        ...
        Action<I2PStream, byte[]> dataReceivedHandler = (stream, data) => { i2pDataQueue.Writer.TryWrite(data); };
        ...
        finally
        {
            ...
            try
            {
                await Task.WhenAll(tcpToI2p, i2pToTcpWorker);
            }
            catch
            {
            }
    ...
    /// </summary>
    private async Task ForwardToOutproxy(NetworkStream clientStream, string requestLine, string target,
        byte[] headerData, CancellationToken ct)
    {
        try
        {
        ...
        catch (Exception ex)
        {
            ...
            try
            {
                await SendError(clientStream, 502, "Outproxy connection failed");
            }
            catch
            {
            }
    ...
    /// <summary>
    ///     Log encryption and LeaseSet info for the remote destination to the proxy log.
    ///     Surfaces garlic/session encryption details to help diagnose connectivity issues.
    /// </summary>
    private static string KeyTypeName(I2PKeyType.KeyTypes? kt)
    {
        return kt switch
        {
            I2PKeyType.KeyTypes.ElGamal2048 => "ElGamal",
            I2PKeyType.KeyTypes.X25519 => "ECIES-X25519",
            I2PKeyType.KeyTypes.MLKEM512_X25519 => "MLKEM512+X25519",
            I2PKeyType.KeyTypes.MLKEM768_X25519 => "MLKEM768+X25519",
            I2PKeyType.KeyTypes.MLKEM1024_X25519 => "MLKEM1024+X25519",
            _ => kt?.ToString() ?? "?"
        };
    }

    private void LogRemoteLeaseSetInfo(string method, string target, string hostname, I2PDestination destination)
    {
        try
        {
            var destHash = destination.IdentHash;

            // Our local LeaseSet encryption keys (what we publish)
            var localKeys = ClientDestination.PublicKeys?
                .Select(pk => KeyTypeName(pk.Certificate?.PublicKeyType))
            ...
            {
                var leaseCount = remoteLeaseSet.Leases?.Count() ?? 0;
                var lsType = remoteLeaseSet.MessageType;
                var lsEncKeys = remoteLeaseSet.PublicKeys?
                    .Select(pk => KeyTypeName(pk.Certificate?.PublicKeyType))
                    .ToArray() ?? Array.Empty<string>();

                var expiry = remoteLeaseSet.Expire;
                var expiresIn = expiry > DateTime.UtcNow
                    ? $"expires in {(expiry - DateTime.UtcNow).TotalSeconds:F0}s"
                    : $"EXPIRED {(DateTime.UtcNow - expiry).TotalSeconds:F0}s ago";

                leaseSetInfo =
                    $"LS({lsType}): {leaseCount} leases, remote keys=[{string.Join(", ", lsEncKeys)}], {expiresIn}";
            }
            else
            {
                leaseSetInfo = "No cached LeaseSet";
            }

            HttpProxyLogger.Inst.Log(
                method,
                target,
                "Encryption",
                $"Local keys=[{string.Join(", ", localKeys)}] | {destHash.Id32Short}: {leaseSetInfo}");
        }
        catch (Exception ex)
        {
            HttpProxyLogger.Inst.Log(method, target, "Encryption", $"Failed to read encryption info: {ex.Message}");
        }
    }
}