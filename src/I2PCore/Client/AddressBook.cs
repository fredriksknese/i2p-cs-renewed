...
namespace I2PCore.Client
{
    public class AddressBook
    {
        ...
        public I2PDestination Lookup( string hostname )
        {
            ...
            if ( hostname.EndsWith( ".b32.i2p", StringComparison.Ordinal ) )
            {
                return LookupBlinded( hostname );
            }

            // Ensure .i2p suffix for standard lookups
            if ( !hostname.EndsWith( ".i2p", StringComparison.Ordinal ) )
            {
                hostname += ".i2p";
            }
        ...
        public void AddSubscription( string url )
        {
            ...
            {
                if ( Subscriptions.All( s => s.Url != url ) )
                {
                    Subscriptions.Add( new SubscriptionInfo { Url = url } );
                }
            }
        }
        ...
        private bool EnsureFetchDestination()
        {
            ...
            try
            {
                ...
                _fetchStreaming.SetSendCallback( ( dest, data ) =>
                {
                    _fetchDestination.Send( dest, data );
                } );
