...
public class SOCKSProxy : IDisposable
{
    ...
    private async Task HandleSocks4(NetworkStream stream, CancellationToken ct)
    {
        ...
        var cmd = header[0];
        var port = (ushort)((header[1] << 8) | header[2]);
    ...
    private async Task HandleSocks5(NetworkStream stream, CancellationToken ct)
    {
        ...
        if (!hasNoAuth)
        {
            ...
            await stream.WriteAsync(new[] { SOCKS5_VERSION, SOCKS5_AUTH_NO_ACCEPTABLE }, 0, 2, ct);
            return;
        }

        // Select no-auth
        await stream.WriteAsync(new[] { SOCKS5_VERSION, SOCKS5_AUTH_NONE }, 0, 2, ct);
    ...
    /// </summary>
    private static async Task<int> ReadExact(NetworkStream stream, byte[] buffer, int offset, int count,
