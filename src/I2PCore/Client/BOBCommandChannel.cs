...
internal class BOBDestination
{
    ...
    public void Start()
    {
        ...
        if (DestinationInfo == null) throw new InvalidOperationException("No keys set for this destination");
...
internal class BOBClientHandler
{
    ...
    private async Task HandleList()
    {
        ...
        foreach (var kvp in _channel.GetAllDestinations())
        {
            ...
            sb.AppendLine(
                $"DATA NICKNAME: {kvp.Key} STARTING: false RUNNING: {d.IsRunning} STOPPING: false KEYS: {(d.DestinationInfo != null ? "true" : "false")} QUIET: {d.Quiet} INPORT: {d.InPort} INHOST: {d.InHost} OUTPORT: {d.OutPort} OUTHOST: {d.OutHost}");
        }
    ...
    {
        if (string.IsNullOrWhiteSpace(nick)) nick = _currentNick;
