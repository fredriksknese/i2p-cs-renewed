...
namespace I2PCore.Client
{
    ...
    public class ClientContext
    {
        ...
        public void LoadTunnelsConfig()
        {
            var searchPaths = new[]
            {
                System.IO.Path.Combine( RouterContext.RouterPath, "tunnels.conf" ),
                System.IO.Path.Combine( AppDomain.CurrentDomain.BaseDirectory, "tunnels.conf" ),
                System.IO.Path.Combine( Environment.GetFolderPath( Environment.SpecialFolder.UserProfile ), ".i2pd", "tunnels.conf" ),
                System.IO.Path.Combine( Environment.GetFolderPath( Environment.SpecialFolder.UserProfile ), ".i2p-cs", "tunnels.conf" ),
            };
        ...
        private void StartAddressBook()
        {
            ...
            if ( !string.IsNullOrWhiteSpace( subscriptions ) )
            {
                foreach ( var url in subscriptions.Split( ',', ';' ) )
                {
                    ...
                    {
                        AddressBook.AddSubscription( trimmed );
                    }
                }
            }
        ...
        private void EnsureSharedProxyDestination()
        {
            ...
            {
                _sharedProxyDestination.Send( dest, data );
            } );
        ...
        private void StartHTTPProxy()
        {
            ...
            try
            {
                ...
                {
                    HTTPProxy.OutproxyUrl = outproxy;
                }
        ...
        private void StartSOCKSProxy()
        {
            ...
            try
            {
                ...
                SOCKSProxy.ResolveHostname = hostname =>
                {
                    return AddressBook?.Lookup( hostname );
                };
