...
namespace I2PCore.Crypto
{
    ...
    public class SHA3_256
    {
        ...
        public void Update(byte[] data, int offset, int length)
        {
            ...
            while (remaining > 0)
            {
                int copyLen = Math.Min(remaining, Rate - bufferPos);
        ...
        public byte[] Final()
        {
            ...
            for (int i = bufferPos + 1; i < Rate; i++)
            {
                buffer[i] = 0;
            }
            buffer[Rate - 1] |= 0x80;
        ...
        private void AbsorbBlock()
        {
            ...
            {
                state[i] ^= BytesToLane(buffer, i * 8);
            }
        ...
        {
            int laneCount = (length + 7) / 8;
            for (int i = 0; i < laneCount && i * 8 < length; i++)
            {
        ...
        private static void LaneToBytes(ulong lane, byte[] output, int offset, int length)
        {
            ...
            {
                output[offset + i] = (byte)((lane >> (8 * i)) & 0xFF);
            }
        }
    }
    ...
    {
        public static readonly int[] RotationOffsets = new int[]
        {
             0,  1, 62, 28, 27,
            36, 44,  6, 55, 20,
             3, 10, 43, 25, 39,
        ...
        public static readonly ulong[] RoundConstants = new ulong[]
        {
            0x0000000000000001UL, 0x0000000000008082UL, 0x800000000000808AUL,
            0x8000000080008000UL, 0x000000000000808BUL, 0x0000000080000001UL,
            0x8000000080008081UL, 0x8000000000008009UL, 0x000000000000008AUL,
