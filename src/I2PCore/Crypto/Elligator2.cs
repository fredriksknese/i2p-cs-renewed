...
namespace I2PCore.Crypto
{
    ...
    public static class Elligator2
    {
        ...
        public static byte[] Encode(byte[] publicKey)
        {
            ...
            byte rand = (byte)BufUtils.RandomUint();
            ...
            if (result == null)
            {
                throw new InvalidOperationException("Public key cannot be Elligator2 encoded. Use GenerateEncodablePrivateKey()");
            }
        ...
        private static byte[] InternalEncode(byte[] publicKey, bool alternative, byte highBits)
        {
            ...
            BigInteger x = new BigInteger(1, ReverseBytes(publicKey));
            ...
            BigInteger negativePlusXA = x.Add(A).Negate().Mod(P);

            // val = -ux(x + A) (mod p)
            BigInteger val = U.Multiply(x).Mod(P);
        ...
        public static byte[] Decode(byte[] representative)
        {
            ...
            byte[] rBytes = (byte[])representative.Clone();
            rBytes[31] &= 0x3F; // Mask out high two bits

            BigInteger r = new BigInteger(1, ReverseBytes(rBytes));
            ...
            // v = -A / (1 + ur^2) (mod p)
            BigInteger v = r.Multiply(r).Mod(P);
            ...
            BigInteger plusVA = v.Add(A).Mod(P);
            BigInteger t = v.Multiply(v).Mod(P);
        ...
        private static int Legendre(BigInteger a)
        {
            ...
            BigInteger mp = a.ModPow(DivideMinusP12, P);
            return mp.Equals(BigInteger.One) ? 1 : -1;
        }

        private static BigInteger ModSqrt(BigInteger x)
        {
            BigInteger t = x.ModPow(DivideMinusP14, P);
            BigInteger result = x.ModPow(DividePlusP38, P);

            // If t = -1 (mod p)
            if (t.Add(BigInteger.One).Mod(P).Equals(BigInteger.Zero))
            {
                result = result.Multiply(SqrtM1).Mod(P);
            }

            // If result > (p - 1) / 2, use -result
            if (result.CompareTo(DivideMinusP12) > 0)
            {
        ...
        private static byte[] ReverseBytes(byte[] bytes)
        {
            ...
            for (int i = 0; i < bytes.Length; i++) rev[i] = bytes[bytes.Length - 1 - i];
            return rev;
        }
