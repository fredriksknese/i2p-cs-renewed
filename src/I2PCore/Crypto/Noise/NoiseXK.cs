...
public class NoiseXK
{
    ...
    public ulong SendNonce { get; private set; }

    /// <summary>Current receive nonce counter (for diagnostics)</summary>
    public ulong ReceiveNonce { get; private set; }
    ...
    ///     Uses the currently stored ephemeral keys (must call GenerateEphemeralKeys first)
    /// </summary>
    public (byte[] ephemeralKey, byte[] encryptedPayload) CreateMessage1WithHeaderAndCurrentKeys(byte[] header,
        byte[] payload)
    {
        if (aliceEphemeralPublicKey == null || aliceEphemeralPrivateKey == null)
            throw new InvalidOperationException(
                "Must call GenerateEphemeralKeys before CreateMessage1WithHeaderAndCurrentKeys");
    ...
    ///     Uses the currently stored ephemeral keys (must call GenerateBobEphemeralKeys first)
    /// </summary>
    public (byte[] ephemeralKey, byte[] encryptedPayload) CreateMessage2WithHeaderAndCurrentKeys(byte[] header,
        byte[] payload)
    {
        if (bobEphemeralPublicKey == null || bobEphemeralPrivateKey == null)
            throw new InvalidOperationException(
                "Must call GenerateBobEphemeralKeys before CreateMessage2WithHeaderAndCurrentKeys");
    ...
    public byte[] CreateMessage3Part2(byte[] payload)
    {
        ...
        Logging.LogDebug(
            $"NoiseXK Msg3Part2: key={BitConverter.ToString(key).Replace("-", "").Substring(0, 32)}..., AD(h)={BitConverter.ToString(associatedData).Replace("-", "").Substring(0, 32)}..., payloadLen={payload.Length}, nonce=0");
    ...
    ///     Uses the ephemeral keys generated by GenerateBobEphemeralKeys().
    /// </summary>
    public byte[] PerformMessage2EphemeralAndEE(bool isPQ = false)
    {
    ...
    /// </summary>
    public byte[] GetBobEphemeralPublicKey()
    {
        return bobEphemeralPublicKey;
    }
    ...
    /// </summary>
    public byte[] GetPreSplitChainingKey()
    {
        return PreSplitChainingKey;
    }

    /// <summary>
    ///     Get the hash as it was right before Split().
    /// </summary>
    public byte[] GetPreSplitHash()
    {
        return PreSplitHash;
    }
