...
namespace I2PCore.Data
{
    public class I2PSignature : I2PType
    {
        ...
        public static bool DoVerifyDsaSha1( IEnumerable<BufLen> bufs, I2PSigningPublicKey key, I2PSignature signed )
        {
            ...
            {
                Logging.LogWarning( $"I2PSignature: Unsupported signature type {signed.Certificate.SignatureType} in DSA verify" );
                return false;
            }
            ...
            X9ECParameters ecparam )
        {
            if ( !SupportedSignatureType( signed.Certificate.SignatureType ) )
            {
                Logging.LogWarning( $"I2PSignature: Unsupported signature type {signed.Certificate.SignatureType} in ECDSA verify" );
                return false;
            }
        ...
        public static byte[] DoSignRsa( IEnumerable<BufLen> bufs, I2PSigningPrivateKey key, IDigest digest )
        {
            ...
            var signer = new Org.BouncyCastle.Crypto.Signers.RsaDigestSigner( digest );
        ...
        public static bool DoVerifyGost( IEnumerable<BufLen> bufs, I2PSigningPublicKey key, I2PSignature signed, int bits )
        {
            try
            {
                ...
                if ( bits == 256 )
                {
                    curveOid = Org.BouncyCastle.Asn1.CryptoPro.ECGost3410NamedCurves.GetOid( "Tc26-Gost-3410-12-256-paramSetA" );
                    digest = new Org.BouncyCastle.Crypto.Digests.Gost3411_2012_256Digest();
                }
                else
                {
                    curveOid = Org.BouncyCastle.Asn1.CryptoPro.ECGost3410NamedCurves.GetOid( "Tc26-Gost-3410-12-512-paramSetA" );
                    digest = new Org.BouncyCastle.Crypto.Digests.Gost3411_2012_512Digest();
                }

                var ecParams = Org.BouncyCastle.Asn1.CryptoPro.ECGost3410NamedCurves.GetByOid( curveOid );
                var curve = ecParams.Curve;
                int halfLen = keyBytes.Length / 2;
                ...
                var pubKeyParams = new Org.BouncyCastle.Crypto.Parameters.ECPublicKeyParameters(
                    "ECGOST3410-2012",
                    point,
                    domainParams );

                var signer = new Org.BouncyCastle.Crypto.Signers.ECGost3410Signer();
                signer.Init( false, pubKeyParams );
