...
namespace I2PCore.Data
{
    ...
    public class I2PEncryptedLeaseSet : I2PType, ILeaseSet
    {
        ...
        {
            get => DatabaseStoreMessage.MessageContent.EncryptedLeaseSet;
        }
        ...
        public bool TryDecrypt(BlindedPublicKey blindingKey, out ILeaseSet leaseSet, byte[] clientSecret = null)
        {
            ...
            try
            {
                ...
                int lenOuterPlaintext = encData.Length - 32;
                ...
                int outerOffset = 0;
                byte layer1Flags = outerPlaintext[outerOffset++];
                int authDataLen = 0;
                ...
                // Layer 2 decryption
                int innerCiphertextOffset = outerOffset + authDataLen;
                ...
                int lenInnerPlaintext = lenOuterPlaintext - innerCiphertextOffset - 32;
            ...
            out byte[] innerInput)
        {
            ...
            byte authType = (byte)((buf[offset] >> 1) & 0x07); // bits 1-3: scheme 0=DH, 1=PSK
            int pos = 0;
            ...
            if (authType == 0) // DH
            {
                ...
                ushort numClients = (ushort)((buf[offset + pos] << 8) | buf[offset + pos + 1]);
                pos += 2;
        ...
        public I2PDestination Destination
        {
            get
            {
                ...
                return DecryptedLeaseSet?.Destination;
            }
        }

        public DateTime Expire => (DateTime)Published + TimeSpan.FromSeconds(ExpiresSeconds);

        public System.Collections.Generic.IEnumerable<ILease> Leases
        {
            get
            {
                return DecryptedLeaseSet?.Leases ?? 
                    System.Linq.Enumerable.Empty<ILease>();
            }
        }

        public System.Collections.Generic.IEnumerable<I2PPublicKey> PublicKeys
        {
            get
            {
                return DecryptedLeaseSet?.PublicKeys ?? 
                    System.Linq.Enumerable.Empty<I2PPublicKey>();
            }
        }
