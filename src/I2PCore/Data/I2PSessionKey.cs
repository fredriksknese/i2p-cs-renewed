...
public class I2PSessionKey : I2PType, IComparable<I2PSessionKey>, IEqualityComparer<I2PSessionKey>
{
    ...
    {
        if (buf is null) throw new ArgumentException("SessionKey must be 32 bytes");

        Key = buf.ReadBlock(32);

        if (Key.Length != 32) throw new ArgumentException("SessionKey must be 32 bytes");
    }

    public I2PSessionKey(I2PByteBlock buf)
    {
        if (buf.Length != 32) throw new ArgumentException("SessionKey must be 32 bytes");
        Key = buf;
    }
