...
public class BlindedPublicKey
{
    ...
    {
        var addr = BufUtils.Base32ToByteArray(b33);
        ...
        var checksum = Crc32(addr, 3, addr.Length - 3);
        ...
        var pubKeyLen = I2PSigningKey.SigningPublicKeyLength(SigType);
    ...
    public string ToB33()
    {
        ...
        var checksum = Crc32(addr, 3, PublicKey.Length);
        addr[0] ^= (byte)(checksum & 0xFF);
        addr[1] ^= (byte)((checksum >> 8) & 0xFF);
        addr[2] ^= (byte)((checksum >> 16) & 0xFF);

        return BufUtils.ToBase32String(addr);
    }
